...
1. For DataView and typed-array setters, apply the specified argument coercion
   ...
   the target element conversion such as `ToInt16`.
...
`JsDataView` method still used raw `TryGetDouble` and C# casts. The durable
lesson is that numeric setter semantics are shared runtime behavior, not a
single prototype-method detail. Open-coded casts make it easy for signed zero,
