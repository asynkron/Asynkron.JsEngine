...
| 2026-06-02 | `pre-gate:hasParameterExpressions` plus bounded `pre-gate:usesArguments` | Ordinary sync functions with simple literal defaults now enter production unified bytecode when the existing implicit-arguments read predicate owns the body, including `typeof arguments`, `arguments.length`, and `arguments[0]` in simple expression spans. Broader arguments mutation, call-target, and default-expression combinations remain separately owned. | Focused default-plus-implicit-arguments route-hit tests; `UnifiedBytecodeProduction` pack. |
