...
7. **Performance follow-ups need measured owner receivers.** gh3530 and gh3531 start from recent measured wins, but their next packets are reprofile-and-scope work. gh3530 owns one remaining `functioncalls` production call-dispatch owner after ADR 0378's plan-pure dependency-scan cache; gh3531 owns one remaining `classdef` constructor-dispatch owner after PR #3505's class-constructor slot-storage cache. Neither packet may collapse descriptor/runtime/call-argument, constructor/super dispatch, property-store, or callback costs into one broad performance claim.
...
- **gh3530 functioncalls call-dispatch reprofile:** Performance owns a fresh residual-owner profile after ADR 0378's plan-pure dependency-scan cache. The receiving contract is one measured `functioncalls` owner row that keeps descriptor lookup, runtime invocation, call-argument handling, and the dependency-scan cache boundary separate.
...
| `functioncalls` call-dispatch reprofile after ADR 0378 | Separate packet | Keep descriptor lookup, runtime invocation, call-argument handling, and dependency-scan cache effects measured as distinct owners under gh3530 |
