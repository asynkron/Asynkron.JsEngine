...
| `ObjectExpression` | `shape-dependent` | `TryCompileObjectExpression`; unsupported member kind -> `UnsupportedObjectMemberKind`; static key normalization now covers parser literals plus identifier/literal AST key nodes; remaining invalid static/computed key shapes -> `UnsupportedStaticObjectPropertyName` / `InvalidComputedObjectKey` | `ExpressionProgramLoweringTests.ObjectExpression_StaticIdentifierKeyNode_IsLoweredToExpressionProgram` |
