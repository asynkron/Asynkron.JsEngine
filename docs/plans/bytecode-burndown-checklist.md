...
_Checklist status: 96 / 145 complete. The latest B36 slice admitted sync-generator direct root hoisted helper declarations that capture root body locals via the materialized resumable body environment. B23 remains partial for async captured literals and lexical/private closure contexts; B36 remains partial for async/async-generator captured helpers, recursive/sibling helper graphs, block/Annex B declarations, and class declarations. The resumable opcode gap inventory remains 19 and the instruction gap inventory remains 9._
