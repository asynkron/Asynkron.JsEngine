...
- [x] **P0.3** Diff `UnifiedBytecodeOpCode` enum vs the sync admit-switch (E3) and the two resumable allowlists; name every enum/instruction-but-not-admitted gap. ✅ (2026-06-05): contract now drift-checks sync prototype opcode guard gaps (6 resumable-only opcodes), resumable opcode allowlist gaps (29 opcodes after B26 dynamic reference-store admission), and resumable instruction allowlist gaps (12 instruction records) against source.
...
- [ ] **B29** Dynamic reference plumbing (compound free-var ops) — ☑/☐ — ⚠️ INVESTIGATED-DECLINE (commit 0c2707532 + B26 follow-up): declines at the `CompoundAssignmentSlotInstruction` / `LogicalCompoundAssignmentSlotInstruction` plan-shape gates. The B26 reference stack is now available, but compound/logical free-var ops still need separate read-modify-write admission proof. Correct via IR.
...
| B — Resumable parity + suspension | 57 | 34 complete / 23 open; class-expression decomposition added B24a-B24i |
...
**Status (129 concrete A+B+C checklist items):** 80 complete / 49 open. **Resumable is the bulk of the remaining work; class-expression creation, async-generator delegation, driver state, and fallback-tier retirement remain significant gaps.**
...
_Checklist status: 91 / 145 complete. The latest B26 slice admitted resumable free/global plain writes by persisting dynamic assignment references on the resume state; B29 compound/logical free-var writes remain open._
