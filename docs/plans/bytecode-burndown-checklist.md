...
- [ ] **B36** Nested function/class declaration hoisting in resumable body — ☑/◐ — ◐ partial: direct root, non-capturing function-scoped declarations now route in generators, async functions, and async generators. The resumable invokers pre-populate the declared helper's VM flat slot before `ExecuteResumable`; slot resolution is performed against the compiled `UnifiedBytecodeProgram` so async synthetic resume slots cannot shift the helper into the wrong slot. Remaining B36 work: capturing helpers (declaration graph ordering and recursive/sibling helper dependencies are still unowned even though B23 generator literals now have a materialized body environment), dynamic/eval helpers, descriptor-backed block/Annex B declarations (A43 foundation), and class declarations. Correct for the remaining subset via IR.
