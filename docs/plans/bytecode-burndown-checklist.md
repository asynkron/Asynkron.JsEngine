...
- [x] **A44** PushEnvironment for iterating / non-flat-slot lexical block (per-iter `let`) — *UnsupportedPlanShape* — ☑/☑ — `:599` ✅ STAGE 1 ADMITTED: captured per-iteration `let`/`const` loop bindings now route through production unified bytecode. `SlotAssignmentRewriter` eagerly maps PushEnvironment lexical slots and shares the loop-head/per-iteration flat slot for copy-listed bindings; the compiled `UnifiedBytecodeScopeDescriptor` carries per-iteration copy slot metadata; the VM snapshots those slots before PushEnvironment rebinding and initializes the fresh scope environment instead of TDZ-wiping copied slots. Pinned by `A44PerIterationLetDeclineTests` route assertions plus loop/block scope regression guards. A43 Annex-B block-function admission and B23 resumable nested-function capture remain separate foundation items.
