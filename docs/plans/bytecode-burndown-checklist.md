...
- [ ] **B36** Nested function/class declaration hoisting in resumable body — ☑/◐ — ◐ partial: direct root, non-capturing function-scoped declarations now route in generators, async functions, and async generators. The resumable invokers pre-populate the declared helper's VM flat slot before `ExecuteResumable`; slot resolution is performed against the compiled `UnifiedBytecodeProgram` so async synthetic resume slots cannot shift the helper into the wrong slot. Remaining B36 work: capturing helpers (body locals still live only in resume-state flat slots), recursive/sibling helper dependencies, dynamic/eval helpers, descriptor-backed block/Annex B declarations (A43 foundation), and class declarations. Correct for the remaining subset via IR until the resume state owns a materialized body environment.
