...
- [x] **B24a** Class expression constructor/default-constructor creation — sync ◐ / res ☑ ✅ (Codex): constructor-only class expressions now route through `ExecuteResumable` via `LoadClassLiteral`. The resumable handler creates the class value with the captured calling environment and preserves explicit constructors, implicit base constructors, and implicit derived constructors whose `extends` value is environment-resolved; fields, static blocks/elements, private members, accessors, computed members, member-super shapes, and `extends` expressions that read resumable activation slots still decline for B24b-B24i / later class-definition environment work.
...
- [x] **B46** Resumable opcode-allowlist default (master opcode-level gap) *(→ P0.3)* — n/a/n/a ✅ decomposed by P0.3: the 20 non-allowlisted opcodes are now named and drift-checked under `Resumable Opcode Allowlist Gaps`; the admitted allowlist remains 1:1 with `ExecuteResumable`.
