...
- [ ] **A2** Unresolved dynamic activation chain — *CapturedOrDynamicActivation* — sync ◐ / res ☐ — `:418` — bounded non-with direct-eval activation plus ordinary free dynamic read/store/call is admitted on the sync production VM when the function has no captured activation, no live `with` object in the closure chain, and no arguments-object dependency (`DirectEvalThenOrdinaryDynamicReadStoreCall_UsesUnifiedBytecodeProductionFastPath`). Remaining residue: closure-created live `with` chains / with-backed captured receivers, captured dynamic activation chains, arguments-object-dependent direct-eval shapes outside the owned arguments route, and async/generator parity.
