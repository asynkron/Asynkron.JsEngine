...
| B24h class-expression environment bridge | new B24h child per remaining anchor; gh3490 is closed precedent only | Keep B24h class-expression work separate from B36 class declarations and E5 runner retirement. gh3490 delivered the nested class-literal activation-capture proof; it is not the receiving branch for the remaining open B24h anchors. Replace one remaining B24h open anchor at a time with focused route or no-route proof. | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeResumableClassExpressionTests"`; `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~BytecodeProofManifestTests"` |
...
## Active Sibling Ownership

The active sibling rows named by the clarification remain outside gh3495:
gh3491 owns E5 async-function route parity, gh3624 owns the B36 static-block
fallback classification, and gh3625 owns the B36 deferred class-definition
environment boundary. E5c and remaining B24h anchors need explicitly created
child tasks before implementation work starts.
