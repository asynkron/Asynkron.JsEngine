...
- Unified bytecode property reads now execute through the current accepted production boundary: direct named (`box.value`), exact two-hop named (`box.child.value`), and first-boundary direct computed (`box[key]`) shapes. Accepted programs stay inside owned unified opcodes (`LoadSlot`, `LoadLiteral`, `StoreSlot`, supported `Binary`, `RequireObjectCoercible`, `ResolvePropertyKey`, `GetNamedProperty`, `GetComputedProperty`, `Jump`, `JumpIfFalse`, `Return`) with string-constant/operand-table ownership in `UnifiedBytecodeProgram`, and no bridge back to `ExpressionProgram`, `ExecutionPlanRunner`, or AST evaluation (`docs/adrs/0221-keep-unified-bytecode-property-reads-vm-owned-and-observable.md`, `docs/adrs/0222-keep-unified-bytecode-two-hop-named-property-read-boundary-owned.md`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionEligibilityTests.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionInvocationTests.cs`).
...
- Property-read production coverage remains intentionally narrow: optional chaining, computed-in-chain neighbors, richer computed-key expressions, call/construct/write/update/delete families, `super`/`this`-bound member access, dynamic lookup, captured activation shapes, and out-of-boundary computed reads still decline before unified VM execution under the current selector boundary (`src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `docs/adrs/0218-keep-unified-bytecode-property-read-production-boundary-selector-owned.md`).
...
- Module dependency evaluation still needs explicit task/fault propagation hardening follow-through under ADR 0212 boundaries, especially around returned `EnsureModuleEvaluatedAsync(...)` tasks versus stored `ModuleEntry.EvaluationTask` ownership in dependency drains (follow-up [#2401](https://github.com/asynkron/Asynkron.JsEngine/issues/2401)).
...
40. Continue module-runtime follow-through by hardening dependency evaluation task/fault propagation under ADR 0212, preserving synchronous fault visibility and async continuation tracking without changing public compatibility boundaries (tracked in [#2401](https://github.com/asynkron/Asynkron.JsEngine/issues/2401)).
