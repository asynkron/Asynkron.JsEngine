...
- [ ] (open, gh2678) Continue the unified-bytecode driver-state widening: Slice A (TDZ head environments for sync iterator/for-in drivers) landed via `TdzHeadInit` and ADR 0286; Slice B (awaited iterator/for-in sources) and Slice C (async iterator driver kind) remain declined.
