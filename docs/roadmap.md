...
33. Preserve the unified bytecode production-routing boundary from ADR 0210 while widening sync-production eligibility only through narrow parity/profile-proven selector expansions beyond the current accepted opcode/control-flow subset (tracked in [#2227](https://github.com/asynkron/Asynkron.JsEngine/issues/2227), with control-flow prototype boundary context in [#2182](https://github.com/asynkron/Asynkron.JsEngine/issues/2182), the `==` widening follow-up in [#2256](https://github.com/asynkron/Asynkron.JsEngine/issues/2256), and current production evidence in `docs/performance/unified-bytecode-branch-production-routing.md` plus ADR 0210 evidence updates).
