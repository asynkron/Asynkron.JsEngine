...
- Unified bytecode property access now executes through the current accepted production boundary: direct named/computed reads, optional-free activation-resolved named read chains, direct named/computed writes, direct named/computed compound writes, and named/computed prefix/postfix updates. The executable no-spread call boundary is proven for activation-slot identifier callees, direct named member calls with optional-free activation-resolved named receiver chains, and direct computed member calls with shallow activation-resolved receiver chains and simple literal/slot arguments, while eval/spread/construct/super/optional call families and deeper computed-member call neighbors still decline before VM execution. ADR 0261/0264/0275 keep the post-#2495 call boundary explicit and sequence-owned: named/computed member call execution is integrated, final-receiver ownership is explicit, deeper named chains are VM-owned, and remaining wider call families stay deferred. Accepted programs stay inside owned unified opcodes (`LoadSlot`, `LoadLiteral`, `StoreSlot`, `Binary`, `RequireObjectCoercible`, `ResolvePropertyKey`, `GetNamedProperty`, `GetComputedProperty`, `GetNamedPropertyForCompoundSet`, `GetComputedPropertyForCompoundSet`, `SetNamedProperty`, `SetComputedProperty`, `UpdateNamedProperty`, `UpdateComputedProperty`, `PrepareIdentifierCallTarget`, `PrepareNamedCallTarget`, `PrepareComputedCallTarget`, `CallInvocationBoundary`, `Jump`, `JumpIfFalse`, `Return`) with string-constant/call-target/operand-table ownership in `UnifiedBytecodeProgram`, and no bridge back to `ExpressionProgram`, `ExecutionPlanRunner`, or AST evaluation (`docs/adrs/0221-keep-unified-bytecode-property-reads-vm-owned-and-observable.md`, `docs/adrs/0222-keep-unified-bytecode-two-hop-named-property-read-boundary-owned.md`, `docs/adrs/0234-keep-unified-bytecode-property-writes-strict-and-directive-owned.md`, `docs/adrs/0238-keep-unified-bytecode-compound-property-writes-get-for-set-owned.md`, `docs/adrs/0250-keep-unified-bytecode-call-target-prep-boundary-non-executable.md`, `docs/adrs/0261-keep-unified-bytecode-call-invocation-boundary-plan-sliced-and-deferred.md`, `docs/adrs/0262-keep-unified-bytecode-named-member-call-receiver-owned.md`, `docs/adrs/0263-keep-unified-bytecode-computed-member-call-key-and-receiver-owned.md`, `docs/adrs/0264-keep-unified-bytecode-member-call-final-receiver-owned.md`, `docs/adrs/0275-keep-unified-bytecode-named-chains-owned-and-computed-receiver-boundary-shallow.md`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionEligibilityTests.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionInvocationTests.cs`).
- Unified bytecode completion-lane ownership now includes VM-executed `ReturnUndefined`, `Throw`, and accepted discarded-expression forms. That completion boundary stays separate from the executable no-spread call boundary; eval/spread/construct/super/optional call families still decline before VM execution (`docs/adrs/0252-keep-unified-bytecode-completion-lane-vm-owned.md`, `docs/adrs/0250-keep-unified-bytecode-call-target-prep-boundary-non-executable.md`, `docs/adrs/0262-keep-unified-bytecode-named-member-call-receiver-owned.md`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`).
- Unified bytecode current-state documentation boundary is now explicitly synchronized across contract, roadmap, ADRs, and agent rules: supported opcode/decline inventories remain enum-backed, no-mixed-execution stays mandatory for accepted production programs, and next unsupported buckets remain wider eval/spread/construct/super/optional call families, unsupported driver-state subshapes (async iterator drivers, TDZ head environments, awaited iterator/for-in sources, object destructuring, expression-level `ApplyBindingTarget` destructuring, dynamic-name destructuring targets, and non-slot/unified-slot failures) that still decline before VM execution, label-dependent control flow, and dynamic lookup families (`docs/adrs/0256-keep-unified-bytecode-coverage-matrix-and-boundary-docs-synchronized.md`, `docs/unified-bytecode-expansion-contract.md`, `.claude/rules/unified-bytecode-prototypes.md`, [#2574](https://github.com/asynkron/Asynkron.JsEngine/issues/2574), [#2575](https://github.com/asynkron/Asynkron.JsEngine/issues/2575)).
- Ordinary sync production routing now has an explicit source gate: specialized simple-return binary and binary-chain fast paths stay first, accepted production unified-bytecode programs are attempted before `SyncIrCallTrampoline` and generic `ExecutionPlanRunner`, and remaining unsupported buckets are ranked in the expansion contract (`src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncFunctionInvoker.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionInvocationTests.cs`, `docs/unified-bytecode-expansion-contract.md`).
- Unified bytecode production eligibility now admits `this`-dependent ordinary sync functions: the blanket `_homeObject is not null` pre-gate is removed from `CanUseProductionUnifiedBytecodeFastPath` so class instance methods and object-literal methods without super-property opcodes route through the production VM. Sloppy-mode `this` coercion is handled before VM entry via `CoerceThisValueForNonStrict`. Super-using class methods still decline before VM execution via the plan-level `SuperPropertyDependency` gate, and arrow functions still decline via `IsArrowFunction` and `_lexicalThisEnvironment is not null` pre-gates (`docs/adrs/0279-accept-this-dependent-ordinary-sync-in-unified-bytecode.md`, `src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncFunctionInvoker.cs`, `docs/unified-bytecode-expansion-contract.md`, [#2633](https://github.com/asynkron/Asynkron.JsEngine/issues/2633)).
...
- ADR 0185 now makes the direct-eval cache boundary explicit: repeated direct-eval caching remains eval-host-owned, and tagged-template identity correctness across separate eval parses is preserved by reparsing cached template-literal eval sources rather than reusing stale tagged-template descriptor identity (`docs/adrs/0185-keep-direct-eval-program-cache-strictness-and-caller-context-owned.md`, `src/Asynkron.JsEngine/EvalHostFunction.cs`).
- Native error factory fallback semantics are now explicitly bounded under ADR 0270: shared factory plumbing is accepted, but fallback prototype/null-handling policy stays family-specific and realm-sensitive at call sites (`docs/adrs/0270-keep-native-error-factory-fallback-semantics-explicit.md`, `src/Asynkron.JsEngine/StdLib/StandardLibrary.cs`).
- ADR 0118 follow-through remains durable: Array reduce/some helper surfaces stay JsValue-native after `SomeLike` removal, while adjacent iterator helper/descriptor setup work remains a bounded stdlib follow-up surface instead of a parity claim (`docs/adrs/0118-keep-array-reduce-some-result-helpers-jsvalue-native.md`, `src/Asynkron.JsEngine/StdLib/Iterator/IteratorPrototype.cs`).
...
- Module dependency evaluation task/fault propagation is now hardened under ADR 0212: PR #2406 keeps stored `ModuleEntry.EvaluationTask` as the normal async dependency drain owner while awaiting the returned `EnsureModuleEvaluatedAsync(...)` task as a fallback when no stored task exists, with focused sync and async dependency fault tests in `ModuleTests`. The next bounded follow-up issues for this seam remain `ModuleEntry.LastValue` compatibility reduction in [#2374](https://github.com/asynkron/Asynkron.JsEngine/issues/2374) and milestone-level Node.js-competitor module/runtime/host alignment in [#2342](https://github.com/asynkron/Asynkron.JsEngine/issues/2342).
- `arguments.length` direct reads and numeric `arguments[i]` direct reads are now `JsArgumentsObject`-owned fast paths that skip generic property-name lookup while the length property is untouched; any assignment, delete, or `defineProperty` invalidates the direct value and falls back to ordinary object semantics. Numeric-index reads remain descriptor-aware under ADR 0265 and lazy-descriptor-aware under ADR 0211. The focused activation proof pack keeps mutation and prototype fallback explicitly gated (`docs/adrs/0276-keep-arguments-length-direct-read-untouched-and-descriptor-aware.md`, `src/Asynkron.JsEngine/JsTypes/JsArgumentsObject.cs`, `src/Asynkron.JsEngine/Runtime/JsOps.cs`, `docs/performance/activation-arguments-direct-read-fast-path.md`, PR [#2612](https://github.com/asynkron/Asynkron.JsEngine/pull/2612)).
- Resumable unified bytecode now has a real VM-owned state model for accepted async-like and simple generator activations: `Yield`, `StoreResumeValue`, `AwaitAndDiscard`, and `AwaitedReturn` opcodes are in the production resumable route, program counter/operand stack/slots/pending-await/resume-payload/completion state survive suspension through `UnifiedBytecodeVirtualMachine.ExecuteResumable`, and `yield*` remains a pre-VM resumable decline until delegated `.return()` and `.throw()` abrupt-resume is modeled. Accepted resumable programs must not call back into `ExecutionPlanRunner` or AST evaluators after acceptance (`docs/adrs/0277-keep-resumable-unified-bytecode-state-bounded-and-yield-star-declined.md`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, PR [#2622](https://github.com/asynkron/Asynkron.JsEngine/pull/2622)).
- Primary sync route guard now ensures 100% of accepted ordinary sync production programs attempt `UnifiedBytecodeVirtualMachine` before `SyncIrCallTrampoline` and generic `ExecutionPlanRunner`. This is a selector coverage estimate for accepted programs, not a full ECMAScript function-surface claim; unsupported buckets remain pre-VM declines as documented in the expansion contract (`src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncFunctionInvoker.cs`, `docs/unified-bytecode-expansion-contract.md`, PR [#2623](https://github.com/asynkron/Asynkron.JsEngine/pull/2623)).
...
- Property-access and call production coverage remains intentionally narrow beyond the accepted direct boundaries: logical assignments, optional chaining, richer computed-key expressions, deeper computed-member call receiver chains, complex member chains, `super`-property access, private fields, `delete`, eval/spread/construct call families, captured activation shapes, and out-of-boundary computed/member forms still decline before unified VM execution under the current selector boundary (`src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `docs/adrs/0218-keep-unified-bytecode-property-read-production-boundary-selector-owned.md`, `docs/adrs/0234-keep-unified-bytecode-property-writes-strict-and-directive-owned.md`, `docs/adrs/0238-keep-unified-bytecode-compound-property-writes-get-for-set-owned.md`, `docs/adrs/0250-keep-unified-bytecode-call-target-prep-boundary-non-executable.md`, `docs/adrs/0262-keep-unified-bytecode-named-member-call-receiver-owned.md`, `docs/adrs/0275-keep-unified-bytecode-named-chains-owned-and-computed-receiver-boundary-shallow.md`).
...
33. Preserve the unified bytecode production-routing boundary from ADR 0210 while widening sync-production eligibility only through narrow parity/profile-proven selector expansions beyond the current accepted opcode/control-flow subset. Current proven production coverage includes direct named/computed reads, optional-free activation-resolved named read chains, direct named/computed writes, direct named/computed compound writes, direct named/computed updates, completion-lane `ReturnUndefined`/`Throw`/accepted discarded-expression VM execution, integrated completed-lane composition, no-spread activation-slot identifier-call invocation, direct receiver-aware named member-call invocation with optional-free named receiver chains, direct receiver-aware computed member-call invocation with shallow receiver chains and simple literal/slot arguments, and bounded sync driver-state lanes (sync iterator driver, for-in driver, and direct-slot array destructuring driver shapes) under ADR 0218/0221/0222/0234/0238/0250/0252/0253/0255/0258/0261/0262/0263/0264/0275 ownership. Keep next unsupported buckets explicit in every widening slice: wider eval/spread/construct/super/optional call families, deeper computed-member call receiver chains, unsupported driver-state subshapes (async iterator drivers, TDZ head environments, awaited iterator/for-in sources, object destructuring, expression-level `ApplyBindingTarget` destructuring, dynamic-name destructuring targets, and non-slot/unified-slot failures) that still decline before VM execution, label-dependent control flow, and dynamic lookup families. Follow-up remains split between refreshed evidence in [#2340](https://github.com/asynkron/Asynkron.JsEngine/issues/2340), ADR 0261's deferred call-family lanes, model-first for-in/destructuring driver-state planning in [#2496](https://github.com/asynkron/Asynkron.JsEngine/issues/2496), and roadmap boundary-synchronization follow-through in [#2574](https://github.com/asynkron/Asynkron.JsEngine/issues/2574) and [#2575](https://github.com/asynkron/Asynkron.JsEngine/issues/2575). Long-term Node.js-competitor alignment for module/runtime/host seams is tracked in [#2342](https://github.com/asynkron/Asynkron.JsEngine/issues/2342) (also tracked in [#2256](https://github.com/asynkron/Asynkron.JsEngine/issues/2256); prior related slices in [#2227](https://github.com/asynkron/Asynkron.JsEngine/issues/2227), with control-flow prototype boundary context in [#2182](https://github.com/asynkron/Asynkron.JsEngine/issues/2182), current production evidence in `docs/performance/unified-bytecode-branch-production-routing.md`, `docs/performance/propertyaccess-unified-bytecode-production-profile-evidence.md`, and ADR 0210/0218/0234/0238/0250/0252/0258/0261/0262/0263/0264/0275). Keep this widening effort aligned with the #2342 architecture-alignment milestones below.
...
36. Continue bounded stdlib follow-through after ADR 0118 by reducing adjacent IteratorPrototype helper/descriptor setup overhead in one semantics-safe slice with focused regression/profile evidence (tracked in [#2596](https://github.com/asynkron/Asynkron.JsEngine/issues/2596)).
37. Align ProfileRunner `simplearithmetic` workload shape with `benchmark.sh` and isolate remaining IIFE overhead before claiming further simplearithmetic reductions (tracked in [#2281](https://github.com/asynkron/Asynkron.JsEngine/issues/2281)).
38. Continue typed module evaluation follow-through by shrinking `ModuleEntry.LastValue` object-shaped compatibility seams and keeping module execution helpers JsValue-native under ADR 0212 boundaries (tracked in [#2374](https://github.com/asynkron/Asynkron.JsEngine/issues/2374)).
39. Continue host/stdlib follow-through by reducing nearby overhead around Math host fast dispatch and splice-family delete-count ownership without weakening ADR 0227/0228 semantics constraints (tracked in [#2375](https://github.com/asynkron/Asynkron.JsEngine/issues/2375)).
40. Continue activation-arguments follow-through with one narrow semantic-safe slice that targets non-capturing loop-scope environment overhead on `activation-arguments-lite`, using selected-profile evidence and focused activation proofs before any acceptance claim (tracked in [#2402](https://github.com/asynkron/Asynkron.JsEngine/issues/2402)).
41. Continue recursion follow-through after ADR 0241 with one narrow profile-backed slice that widens strict simple numeric self-recursion only where shape/binding guards remain compiler/runtime-proven (tracked in [#2450](https://github.com/asynkron/Asynkron.JsEngine/issues/2450)).
42. Continue JsOps/property cleanup after ADR 0240 by removing one bounded adjacent object-carrier compatibility seam while preserving receiver identity and accessor/prototype observability (tracked in [#2451](https://github.com/asynkron/Asynkron.JsEngine/issues/2451)).
43. Continue native-error factory follow-through after ADR 0270 by tightening realm-sensitive fallback proof coverage and any bounded helper-surface cleanup while preserving per-family prototype and null-fallback semantics boundaries (tracked in [#2606](https://github.com/asynkron/Asynkron.JsEngine/issues/2606), with direct-eval companion follow-through in [#2595](https://github.com/asynkron/Asynkron.JsEngine/issues/2595)).
44. Widen unified bytecode production eligibility to accept `this`-dependent ordinary sync functions in one bounded slice, unlocking class/object methods through the unified VM hot path: require a new ADR, focused proof pack for strict/sloppy/class-method `this` binding, negative decline proof for unsafe shapes, and expansion-contract update before any production claim (tracked in [#2633](https://github.com/asynkron/Asynkron.JsEngine/issues/2633)).
45. Profile the unified bytecode primary sync route to collect evidence for the 100% selector coverage claim: measure accepted vs. fallback counts across representative ProfileRunner workloads, publish a coverage doc, and open a follow-up widening issue for any significant decline bucket found (tracked in [#2634](https://github.com/asynkron/Asynkron.JsEngine/issues/2634)).
