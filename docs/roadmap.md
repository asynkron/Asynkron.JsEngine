...
- Unified bytecode property access now executes through the current accepted production boundary: direct named/computed reads, exact two-hop named reads, direct named/computed writes, direct named/computed compound writes, and named/computed prefix/postfix updates. The first executable no-spread identifier-call boundary is also proven for activation-slot callees with simple literal/slot arguments, while member/computed/eval/spread/construct/super/optional call families still decline before VM execution. ADR 0261 keeps the post-#2495 call boundary explicit and sequence-owned: the next widening slice is named member call execution ([#2530](https://github.com/asynkron/Asynkron.JsEngine/issues/2530)), followed by computed member call execution ([#2531](https://github.com/asynkron/Asynkron.JsEngine/issues/2531)). Accepted programs stay inside owned unified opcodes (`LoadSlot`, `LoadLiteral`, `StoreSlot`, `Binary`, `RequireObjectCoercible`, `ResolvePropertyKey`, `GetNamedProperty`, `GetComputedProperty`, `GetNamedPropertyForCompoundSet`, `GetComputedPropertyForCompoundSet`, `SetNamedProperty`, `SetComputedProperty`, `UpdateNamedProperty`, `UpdateComputedProperty`, `PrepareIdentifierCallTarget`, `CallInvocationBoundary`, `Jump`, `JumpIfFalse`, `Return`) with string-constant/call-target/operand-table ownership in `UnifiedBytecodeProgram`, and no bridge back to `ExpressionProgram`, `ExecutionPlanRunner`, or AST evaluation (`docs/adrs/0221-keep-unified-bytecode-property-reads-vm-owned-and-observable.md`, `docs/adrs/0222-keep-unified-bytecode-two-hop-named-property-read-boundary-owned.md`, `docs/adrs/0234-keep-unified-bytecode-property-writes-strict-and-directive-owned.md`, `docs/adrs/0238-keep-unified-bytecode-compound-property-writes-get-for-set-owned.md`, `docs/adrs/0250-keep-unified-bytecode-call-target-prep-boundary-non-executable.md`, `docs/adrs/0261-keep-unified-bytecode-call-invocation-boundary-plan-sliced-and-deferred.md`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionEligibilityTests.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionInvocationTests.cs`).
- Unified bytecode completion-lane ownership now includes VM-executed `ReturnUndefined`, `Throw`, and accepted discarded-expression forms. That completion boundary stays separate from the first executable no-spread identifier-call boundary; the remaining member/computed/eval/spread/construct/super/optional call families still decline before VM execution (`docs/adrs/0252-keep-unified-bytecode-completion-lane-vm-owned.md`, `docs/adrs/0250-keep-unified-bytecode-call-target-prep-boundary-non-executable.md`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`).
- Unified bytecode current-state documentation boundary is now explicitly synchronized across contract, roadmap, ADRs, and agent rules: supported opcode/decline inventories remain enum-backed, no-mixed-execution stays mandatory for accepted production programs, and next unsupported buckets remain wider call families, iterator/destructuring driver-state, label-dependent control flow, and dynamic lookup families (`docs/adrs/0256-keep-unified-bytecode-coverage-matrix-and-boundary-docs-synchronized.md`, `docs/unified-bytecode-expansion-contract.md`, `.claude/rules/unified-bytecode-prototypes.md`).
