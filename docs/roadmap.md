...
- Unified bytecode property reads now execute through the production path for the first accepted boundary: direct named reads and direct computed reads that match the `RequireObjectCoercible(Depth: 1) -> ResolvePropertyKey -> GetComputedProperty` shape, with invocation proof coverage for getter/proxy, primitive boxing, and `ToPropertyKey` semantics (`src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionEligibilityTests.cs`, `tests/Asynkron.JsEngine.Tests/UnifiedBytecodeProductionInvocationTests.cs`).
