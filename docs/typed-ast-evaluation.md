...
complete replacement for `JsProgramEvaluator` yet, but it demonstrates the key ideas:
...
port semantics from `JsProgramEvaluator` into the typed version. Because evaluation is
...
to the legacy `JsProgramEvaluator`. This keeps the public behaviour stable while still
