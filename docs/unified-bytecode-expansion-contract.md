...
| `A51h` | Array/object/template literal, spread source, computed object key, simple literal-span shapes, and restricted literal/span helper diagnostics for unsupported logical-control and conditional consequent/alternate operands; manifest `A51h-*stays-open` rows pin the current source anchors | Existing expression-program literal/spread evaluation | both |
