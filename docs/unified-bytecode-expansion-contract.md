...
- `A51f3:DynamicIdentifierExpressionLoop` - narrowed. Zero-depth
  catch/finally-region free identifier reads now enable the ordinary
  dynamic-name route through `LoadDynamicIdentifier`, matching the existing
  free-call-target exception-region scan. Stores, updates, assignment
  references, deletes, `typeof`, lexical dynamic declarations, active-with-only
  shapes, and materialized-activation dynamic-name ownership remain gated.
...
- `A51f5:PrivateNeighborExpressionLoop` - narrowed. Plain private named reads
  ...
  lowering the private prefix read before `PrepareNamedCallTarget`. Private
  named delete defense/manual-plan diagnostics, chained optional-private
  receiver-neighbor reads, private receiver-neighbor mutation shapes, and
  private super/member call neighbors stay declined until those exact
