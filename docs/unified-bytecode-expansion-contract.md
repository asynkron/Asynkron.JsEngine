...
| `A51c` | Catch binding, lexical dynamic declaration, active-with dynamic-name, and non-flat scope/environment TDZ-head binding storage; manifest `A51c-*stays-open` source-presence rows pin these concrete guards while the row remains open | Existing environment-aware execution-plan runner | both |
