...
- Current executable call support covers
  ...
  simple, simple-binary, or simple-unary array/object/template literal values or elements
  (`{items: [a, b]}`, `[{x}]`, `{total: a + b}`,
  `{value: -x}`, ``{label: `hello ${name}`}``) and object literal spread entries
