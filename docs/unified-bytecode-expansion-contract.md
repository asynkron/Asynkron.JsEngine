...
| `A51k` | Simple binary/unary/control/conditional operand spans; nested admitted literal-value operand spans now route through the production VM, while call/private/dynamic-neighbor operands, unsupported simple-operand helper diagnostics, and conditional simple-literal-span diagnostics remain outside this lane; manifest `A51k-*stays-open` rows pin the simple binary/operator and unsupported simple-operand source anchors | Existing expression-program evaluation | both |
