...
- Dynamic identifier support does not make dynamic activation globally
  ...
  dynamic-name lane is limited to functions without captured activation or live
  with-chain closure state. Declaration-free literal eval may read the caller's
  current activation and bounded implicit `arguments` object; arguments-object
  dependencies outside that owned route, captured dynamic activation,
  with-chain closure residue, and async/generator functions still decline
  before VM execution.
