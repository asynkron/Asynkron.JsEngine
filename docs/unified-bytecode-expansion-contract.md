...
| `A51i` | Computed/optional/private property-read and receiver-boundary shapes; read-only private receiver-prefix chains such as `receiver.#child.value` and `receiver.#child[key]`, private receiver-prefix named calls such as `receiver.#child.value()`, private receiver-prefix public named/computed writes such as `receiver.#child.value = value` and `receiver.#child[key] = value`, plus single-hop optional private reads such as `receiver?.#field`, are now sync-admitted through owned property-read/call-target/write opcodes; unsupported property-read base diagnostics stay here | Existing expression-program property evaluation for remaining private-adjacent property reads | both |
...
- `A51f5:PrivateNeighborExpressionLoop` - narrowed. Plain private named reads
  ...
  terminal. Private named delete defense/manual-plan
  diagnostics, chained optional-private receiver-neighbor reads, private
  receiver-neighbor compound/logical/update/private-terminal writes,
