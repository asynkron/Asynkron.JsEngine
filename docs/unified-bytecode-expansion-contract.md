...
| Standalone expression and binding bridges | `TypedAstEvaluator.ExpressionPrograms`, `TypedAstEvaluator.BindingTargetPrograms`, and `ExecutionPlanRunner.Core` helper entrypoints | Quarantined helper surfaces still construct runners for standalone expression execution and binding-target programs; these are helper-owned bridges, not normal production route callbacks from the VM. The profiler-only expression-program loop is no longer part of this source surface: `ProfileRunner` compiles synthetic profile cases to standalone unified bytecode and executes the unified VM directly, while `ProfileEvaluateExpressionProgramLoop` is source-gated as absent. | `rtk rg -n "EvaluateStandaloneExpressionProgram|ApplyStandaloneBindingTargetProgram" src/Asynkron.JsEngine/Ast`; `rtk rg -n "ProfileEvaluateExpressionProgramLoop" src/Asynkron.JsEngine/Ast` |
