...
- Current checklist rebaseline (2026-07-16): the finite checklist is
  **138 / 161** complete. A+B+C+D are **127 / 146** complete with 19 open rows,
  Phase E is **6 / 10** complete with E4 closed and E5b-E5e open, and the proof
  manifest has 32 items with 18 non-done proof rows. Checklist-open A51g1-A51g4
  and A51i remain explicit child lanes even though they do not yet have manifest
  closure rows.
- Completing the decline-burndown plan did not make unified bytecode the only
  ...
  execution paths.
- E4 is now a source-absence-ratcheted closure row: the old standalone,
  lowered, dynamic, binding-target, and profiler expression-program bridge names
  are tombstoned in the proof manifest. E5 owner inventories remain
  classification evidence, not closure claims. Remaining ordinary work is the
  non-dynamic A/B/C parity and runner-retirement surface named in the checklist
  and proof manifest. Terminal dynamic residue stays separate: multi-argument,
  spread, runtime-source, or declaration-injecting direct eval; awaited `with`
  object evaluation; retained live `with` scopes outside the VM current-
  environment lane; eval-injected runtime bindings; and
...
- Public class methods with active or captured private-name scopes are no longer
  ...
  Plain private named reads also execute as simple value operands in binary/RHS
  spans such as `receiver.#field + delta` and `this.#x = this.#x + a`.
...
|---|---|---|---|
| Ordinary sync fallback | `TypedAstEvaluator.SyncFunctionInvoker.TryInvokeIrFast` and adjacent constructor/class invocation bridges | Production unified bytecode is attempted before specialized parameter/binary routes and `SyncIrCallTrampoline`; the generic simple-return-expression `ExpressionProgram` branch is retired, and remaining declines flow to `ExecutionPlanRunner.ExecuteClassifiedSyncFunctionIrResidue` rather than the removed ordinary fallback-runner helper | `rtk rg -n "TryInvokeIrFast|ExecuteClassifiedSyncFunctionIrResidue|SyncIrCallTrampoline|CreateClassifiedOrdinarySyncFunctionFallbackRunner|CreateClassifiedClassConstructorFallbackRunner|\\.RunSync\\(" src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncFunctionInvoker.cs src/Asynkron.JsEngine/Ast/TypedAstEvaluator.ExecutionPlanRunner.Core.cs` |
| Script fallback | `StatementNodeExtensions.RunOrdinaryScriptViaClassifiedRunnerFallback` and `TypedAstEvaluator.ExecutionPlanRunner.Core.ExecuteClassifiedOrdinaryScriptFallback` | Top-level scripts try the production selector first when eligible. Terminal dynamic direct-eval script residue and eval execution stay on separate classified helpers, while remaining non-terminal script declines flow through the ordinary classified fallback helper; the old `ExecutionPlanRunner.RunScript(` entry name is source-absence-ratcheted | `rtk rg -n "RunOrdinaryScriptViaClassifiedRunnerFallback|ExecuteClassifiedOrdinaryScriptFallback|ExecuteClassifiedTerminalDynamicScriptResidue|ExecuteClassifiedEvalScriptFallback|ExecutionPlanRunner\\.RunScript|RunOrdinaryDeclinedScript" src/Asynkron.JsEngine/Ast/Legacy/StatementNodeExtensions.cs src/Asynkron.JsEngine/Ast/TypedAstEvaluator.ExecutionPlanRunner.Core.cs` |
...
| Generator fallback | `TypedAstEvaluator.IrSyncGeneratorInvoker` and `TypedAstEvaluator.GeneratorFunctionBase` | The old post-`EvaluateResumable` declined-residue runner bridge is retired. Function creation chooses the creation-time IR sync-generator route for simple-parameter bodies that are not resumable-VM eligible and for non-simple parameter shapes that need full FunctionDeclarationInstantiation; this route remains classified separately from E5d declined-residue bridge retirement | `rtk rg -n "IrSyncGeneratorInvoker|new ExecutionPlanRunner|CreateClassifiedSyncGeneratorDeclinedResidueRunner|EvaluateResumable|TryCreateResumableSuperBinding|ResumableSuperBinding" src/Asynkron.JsEngine/Ast/TypedAstEvaluator.IrSyncGeneratorInvoker.cs src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncGeneratorInvoker.cs src/Asynkron.JsEngine/Ast/TypedAstEvaluator.GeneratorFunctionBase.cs` |
...
| Class/static-block bridge | `ClassDefinitionExtensions.ExecuteStaticBlock` | Static-block-only class expressions can route through resumable `LoadClassLiteral`; eligible static-block bodies now execute their cached static-block plan through production unified bytecode before any fallback. The old static-block `ExecutionPlanRunner.RunScript(` call is source-absence-ratcheted; declined static-block bodies log `classified-static-block-ir-fallback` and evaluate the block body through the existing class-definition environment path until non-production static-block plans are owned | `rtk rg -n "ExecuteStaticBlock|TryExecuteStaticBlockViaUnifiedBytecode|classified-static-block-ir-fallback|ExecutionPlanRunner\\.RunScript" src/Asynkron.JsEngine/Ast/ClassDefinitionExtensions.cs docs/plans/bytecode-burndown-checklist.md` |
| Standalone expression and binding bridges | `UnifiedBytecodeExpressionProgramExecutor`, `TypedAstEvaluator.BindingTargetPrograms`, and `ExecutionPlanRunner` helper entrypoints | `EvaluateStandaloneExpressionProgram`, `EvaluateLoweredExpressionProgram`, `EvaluateDynamicExpressionProgram`, `ApplyStandaloneBindingTargetProgram`, and `UnifiedBytecodeExpressionProgramExecutor.ExecuteDynamic` are deleted and source-absence-ratcheted. Standalone `ExpressionProgram` payloads now execute through `UnifiedBytecodeExpressionProgramExecutor.ExecuteStandalone`, which compiles them with `UnifiedBytecodeCompiler.TryCompileStandaloneExpressionProgram` and executes `UnifiedBytecodeVirtualMachine` directly. Legacy value-only operands route through cached lowered programs instead of the old dynamic executor. External lowered binding-target programs call the static lowered binding-target core directly; runner-internal binding-target calls still pass the active runner and remain part of the E5 runner retirement surface. The profiler-only expression-program loop is also gone: `ProfileRunner` compiles synthetic profile cases to standalone unified bytecode and executes the unified VM directly, while `ProfileEvaluateExpressionProgramLoop` is source-gated as absent. | `rtk rg -n "EvaluateStandaloneExpressionProgram|EvaluateLoweredExpressionProgram|EvaluateDynamicExpressionProgram|ApplyStandaloneBindingTargetProgram|UnifiedBytecodeExpressionProgramExecutor\\.ExecuteDynamic|ProfileEvaluateExpressionProgramLoop|ProfileEvaluateLoweredExpressionProgramLoop" src/Asynkron.JsEngine tools/ProfileRunner -g "*.cs"` |
...
|---|---|---|---|
| `A51a1` | Malformed entrypoint and invalid-target integrity guards; unsupported-entrypoint checks are defensive backstops for malformed manually-constructed plans, not active valid-plan declines | None for valid generated plans | both |
| `A51a2` | Loop-shaped topology and remaining loop-control diagnostics; switch-style breakable wrappers are sync-admitted and remaining source-backed loop-control reasons require generated-topology proof | Existing execution-plan runner | both |
| `A51b` | Activation-slot metadata, slot-layout, declaration / assignment / update storage targets, and iterator/for-in TDZ-head flat-slot mapping; manifest `A51b-*stays-open` source-presence rows pin the compiler and eligibility guards while the row remains open | Existing execution-plan runner | both |
| `A51c` | Catch binding, lexical dynamic declaration, active-with dynamic-name, and non-flat scope/environment TDZ-head binding storage; manifest `A51c-*stays-open` source-presence rows pin catch-binding topology, catch-binding slot mapping, lexical dynamic declaration storage, active-with dynamic-name gates, materialized-activation dynamic residue, and the ordinary dynamic-name activation-slot boundary while the row remains open | Existing environment-aware execution-plan runner | both |
...
| `A51g` | Call-target preparation, direct-eval call boundary, member/super/private call-target, invocation-boundary shapes | Existing call/eval/super IR paths | both; terminal dynamic residue remains D1/D2/D4 for unowned direct-eval/eval-injected lanes and `Function(...)`-produced bodies; the `Function` call boundary itself is ordinary work when not producing a dynamic body |
| `A51h` | Object method/accessor/private/name-inference restrictions, logical-control operands, and conditional consequent/alternate operands inside simple literal spans; manifest `A51h-*stays-open` rows pin these non-container residue anchors after the spread/computed-key and literal-container helper splits | Existing expression-program literal/spread evaluation | both |
| `A51h1` | Spread-source spans and computed object-key spans inside array/object literal lowering; manifest `A51h1-*stays-open` rows pin the current spread-source and computed-key source anchors without taking ownership of call/private/dynamic-key work outside the measured span helpers | Existing expression-program literal/spread evaluation | both |
| `A51h2` | Array/object/template literal-container span helper lane covering `TryAppendSimpleArrayLiteralSpan`, `TryAppendSimpleObjectLiteralSpan`, `TryAppendSimpleTemplateLiteralSpan`, and matching `UnifiedBytecodeProductionEligibility` measurers; manifest `A51h2-*stays-open` rows pin the current helper and boundary source anchors | Existing expression-program literal/spread evaluation | both |
| `A51i` | Computed/optional/private property-read and receiver-boundary shapes; read-only private receiver-prefix chains such as `receiver.#child.value` and `receiver.#child[key]`, private receiver-prefix named calls such as `receiver.#child.value()`, private receiver-prefix public named/computed writes such as `receiver.#child.value = value` and `receiver.#child[key] = value`, one-hop private receiver-prefix public named/computed updates such as `receiver.#child.value++` and `receiver.#child[key]++`, plus single-hop optional private reads such as `receiver?.#field`, are now sync-admitted through owned property-read/call-target/write/update opcodes; unsupported property-read base diagnostics stay here | Existing expression-program property evaluation for remaining private-adjacent property reads | both |
| `A51j` | Property write, compound/logical write, update, delete, name-inference, key-span, and RHS-span shapes. The remaining work is split into the A51j child inventory below so each property-mutation lane can be widened or pinned without reopening already-proven computed-prefix and private receiver-prefix work. One-hop private receiver-prefix public named/computed update terminals now route, while deeper private-prefix update chains and compound/logical/private-terminal mutation neighbors remain gated | Existing expression-program mutation evaluation | both |
| `A51k` | Open operand-span diagnostics for unsupported simple binary operators, unsupported simple operands, and call/private/dynamic-neighbor operands. Conditional simple-literal-span diagnostics remain assigned to A51h; call-target, private-name, and dynamic-name neighbor operands stay outside A51k1 pending end-to-end proof of call-target stack, private-name scope, and dynamic-name environment ownership. Manifest `A51k-*stays-open` rows pin both the simple binary/operator anchors and those neighbor-boundary decline anchors | Existing expression-program evaluation | both |
| `A51k1` | Simple value operand-span helper lane for binary, unary, logical-control, and conditional operand spans that compose already-admitted literal-value spans. Owner anchors are `TryAppendSimpleBinaryOperandSpan`, `TryAppendSimpleUnaryOperandSpan`, `TryAppendSimpleNestedOperandSpan`, `TryAppendBoundedSimpleControlExpressionOperandSpan`, and mirrored `UnifiedBytecodeProductionEligibility` walkers including `TryMeasureSimpleLiteralValueOperandSpanCore`, `TryMeasureSimpleBinaryOperandSpan`, `TryMeasureSimpleUnaryOperandSpan`, `TryMeasureSimpleControlExpressionOperandSpan`, and `TryMeasureSimpleConditionalExpressionOperandSpan`; manifest `A51k1-*present` rows pin the helper inventory without taking ownership of call/private/dynamic-neighbor operands | Production VM for admitted simple value operand spans; existing expression-program evaluation for excluded neighbors | both |
...
`A51l` is retired as a standalone catch/try/driver cleanup umbrella. Its former
diagnostics are intentionally mapped to concrete owners instead: catch-binding
and dynamic environment storage to A51c, invalid targets and loop-control
topology to A51a2, iterator/driver state-slot diagnostics to A51d, logical
property-write cleanup-start diagnostics to A51j, and resumable finally-cleanup
guards to explicit eligibility decline rows.

### A51j Property-Mutation Child Inventory

Use these children as the finite A51j retirement queue. Each child must keep
`UnifiedBytecodeCompiler.cs`, `UnifiedBytecodeProductionEligibility.cs`, and
`UnifiedBytecodeVirtualMachine.cs` as the primary owner files when it changes
runtime behavior, then update this contract, `docs/bytecode-progress.md`, and
the focused eligibility/route/proof-manifest tests named below.

| Child | Boundary | Owner files | Proof anchors |
|---|---|---|---|
| `A51j1` public property-write residue | Runtime-widen or pin plain/nested named and computed writes, including simple/nested receiver prefixes and RHS span residue. Preserve already-admitted nested named computed writes, computed-read prefix named writes, computed-read prefix computed writes, literal-base named write chains, rich computed keys, and simple-return dynamic-base writes. Keep optional, call-adjacent, unsupported RHS, multi-computed prefix, and private-neighbor writes out unless the slice proves their stack/evaluation-order contract. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `docs/adrs/0355-admit-computed-prefix-computed-property-mutations-through-owned-spans.md` | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&(FullyQualifiedName~Evaluate_NestedNamedComputedPropertyWrite_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~Evaluate_ComputedPrefixNamedPropertyWrite_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~Evaluate_ComputedPrefixComputedPropertyWrite_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~Evaluate_DoubleComputedPrefixNamedPropertyWrite_DeclinesWithPropertyWriteDependency)"`; `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~ProductionRouteCoverageRatchetTests"` |
| `A51j2` compound/logical property-write residue | Runtime-widen or pin compound/logical public property writes. Preserve nested named computed compound/logical writes and computed-read prefix computed compound/logical writes. Keep name inference, optional/private/call-adjacent prefixes, unsupported key/RHS spans, and unproven multi-computed receiver prefixes declined until a child owns the measured suffix cleanup. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `docs/rules/expression-bytecode-assignment.md` | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&(FullyQualifiedName~Evaluate_NestedNamedComputedCompoundPropertyWrite_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~Evaluate_NestedNamedComputedLogicalPropertyWrite_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~Evaluate_ComputedPrefixComputedCompoundPropertyWrite_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~Evaluate_ComputedPrefixComputedLogicalPropertyWrite_AcceptsOwnedPropertyOpcodes)"`; decline-template drift check in `tests/Asynkron.JsEngine.Tests/BytecodeProofManifestTests.cs` when compiler reasons change |
| `A51j3` property-update residue | Own public named/computed update shapes that still decline after direct updates, simple-return dynamic-base updates, nested named computed updates, computed receiver-prefix updates, and one-hop private receiver-prefix public updates. Do not bundle deeper private-prefix update chains with ordinary public update work. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `docs/rules/ecmascript-private-names.md` | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&(FullyQualifiedName~Evaluate_NestedNamedComputedPropertyUpdate_AcceptsOwnedPropertyOpcodes|FullyQualifiedName~PrivateReceiverPrefix)"`; `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~ProductionRouteCoverageRatchetTests"` |
| `A51j4` delete residue | Own delete shapes under `DeleteDependency`, including optional/computed/dynamic-key boundaries that are not already admitted. Keep parser-early-error private deletes and super-neighbor delete semantics outside ordinary property-mutation widening. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs` | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&FullyQualifiedName~Evaluate_NestedComputedPropertyDeleteDynamicKey_AcceptsOrdinaryDynamicNameOpcode"`; update `tests/Asynkron.JsEngine.Tests/BytecodeProofManifestTests.cs` if the finite decline rows move |
| `A51j5` name-inference quarantine | Keep all member, computed member, super, compound, and logical property writes with assignment name inference declined until the VM owns the semantic split from identifier assignment. Runtime work here must prove member no-inference and parenthesized identifier no-inference separately. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `docs/rules/expression-bytecode-assignment.md` | `rtk rg -n "name inference" src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs docs/unified-bytecode-expansion-contract.md`; focused assignment/name-inference tests before any route widening |
| `A51j6` key-span and RHS-span residue | Own unsupported computed-key and RHS spans for writes, compound/logical writes, updates, and deletes. Preserve the already-admitted rich binary/unary keys, whole-span control-expression keys, simple RHS, admitted literal spans, and ADR 0355 measured receiver/key/value composition. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `docs/adrs/0355-admit-computed-prefix-computed-property-mutations-through-owned-spans.md`, `tests/Asynkron.JsEngine.Tests/BytecodeProofManifestTests.cs` | `rtk rg -n "Unsupported computed property key span|RHS span does not match|Computed property writes with name inference" src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs docs/unified-bytecode-expansion-contract.md`; focused key/RHS eligibility tests for each admitted span |
| `A51j7` private receiver-prefix mutation neighbors | Pin or widen only the private-neighbor mutation shapes not already proven by A51f5/A51j: deeper private-prefix public updates, private receiver-prefix compound/logical writes, private-terminal writes/updates, chained optional-private neighbors, and super/private mutation neighbors. Keep one-hop private receiver-prefix public named/computed writes and updates marked as already admitted. | `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeCompiler.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeProductionEligibility.cs`, `src/Asynkron.JsEngine/Execution/UnifiedBytecode/UnifiedBytecodeVirtualMachine.cs`, `docs/rules/ecmascript-private-names.md` | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~PrivateReceiverPrefix"`; route ratchet rows in `tests/Asynkron.JsEngine.Tests/ProductionRouteCoverageRatchetTests.cs` for admitted private-neighbor cases |
...
- `A51f3:DynamicIdentifierExpressionLoop` - narrowed. Zero-depth
  catch/finally-region free identifier reads, `typeof` free identifiers, and
...
- `A51f5:PrivateNeighborExpressionLoop` - narrowed. Plain private named reads
  now route as simple value operands in binary/RHS spans, and single-hop
  optional private reads route through `GetNamedPropertyOptional`. Private
  receiver-prefix named calls such as `receiver.#child.value()` route by
  lowering the private prefix read before `PrepareNamedCallTarget`. The
  minimal receiver-prefix public named/computed writes (`receiver.#child.value = value`,
  `receiver.#child[key] = value`) route by composing a single non-optional
  private receiver read with a public `SetNamedProperty` / `SetComputedProperty`
  terminal, and the matching one-hop public named/computed updates
  (`receiver.#child.value++`, `receiver.#child[key]++`) route through
  `UpdateNamedProperty` / `UpdateComputedProperty`. Private named delete defense/manual-plan
  diagnostics, chained optional-private receiver-neighbor reads, private
  receiver-neighbor compound/logical/private-terminal writes, deeper
  private-prefix update chains,
  and private super/member call neighbors stay declined until those exact
  private-adjacent routes are VM-owned.
...
| `CapturedOrDynamicActivation` | Activation descriptor `HasCapturedOrDynamicActivation`, including captured dynamic activation outside the admitted captured-closure route or dynamic activation outside the admitted with-backed / non-with direct-eval dynamic-name lanes. The captured-closure lane now admits sync and resumable flat, nested, and colliding captured locals. The sync route admits bounded non-with non-injecting direct-eval activation plus ordinary free dynamic read/store/call when captured dynamic activation and live with-chain dependencies are absent; declaration-free literal eval may read the caller's current activation and bounded implicit `arguments` object. Resumable generator, async, and async-generator routes now admit the same bounded implicit-`arguments` direct-eval reads after materializing the resumable body environment and arguments object. Declaration-bearing, runtime-source, spread/multi-arg eval, captured dynamic activation, and retained live-`with` closure chains remain declined | Existing sync IR / environment route | Dynamic activation lane | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeResumableDynamicIdentifierTests&FullyQualifiedName~DirectEvalArgumentsRead"` |
...
- Dynamic identifier support does not make dynamic activation globally
  ...
  object on sync, generator, async, and async-generator routes; arguments-object
  dependencies outside that owned route, captured
...
- Captured/dynamic activation outside admitted dynamic-name lanes, unbounded
  arguments objects, `new.target`, optional and
