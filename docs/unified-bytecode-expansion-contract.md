...
| `OptionalChainDependency` | Optional chains outside the admitted optional property-read, optional-call, named optional-chain call-argument, and exact optional named/computed delete boundaries; current concrete sync call-argument residue includes computed second-hop shapes such as `fn(box?.[key]?.[key])`, `fn(box?.[key]?.value)`, and `fn(box?.prop?.[key])` | Existing sync IR optional-chain route | Optional-chain widening lane | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&FullyQualifiedName~Evaluate_OptionalChainNamedPrefixPlainCallExpressionPlan_AcceptsExecutableInvocationBoundary"` |
...
| `UnsupportedPlanShape` | Missing activation slot metadata, unsupported instruction families, unsupported compiler shapes, unsupported resumable opcodes, and unknown production opcode defaults. A49 (`Activation slot metadata is required.`): unreachable for any valid compiled function/script plan — `ExecutionPlanBuilder` always populates a non-null `ActivationSlotShape` (and the nested-function restamp carries it forward), so this arm is a pure defensive backstop; trivial plans (top-level expression script, empty `function f(){}`) already route. A41 (`Identifier assignment reference '{name}' resolves to an activation slot and is outside the ordinary dynamic-name production slice.` / `Identifier assignment target '{name}' resolves to an activation slot and is outside the ordinary dynamic-name production slice.`): a slot-bound identifier assigned to as a CONSUMED (value-producing) sub-expression — e.g. `return (x = 5);`, `var y = (x = 5);`, `g(x = 5)`, chained `(x = z = 5)` — lowers to the expression-program `ResolveIdentifierReference`/`StoreResolvedIdentifier` ops instead of a top-level `AssignmentSlotInstruction` → `StoreSlot`. The eligibility arm declines any such op that resolves to an activation slot UNCONDITIONALLY (not gated on the dynamic-name flag or a free callee), because the dynamic-name VM has no slot-targeting reference-store opcode: `StoreDynamicIdentifierReference` writes by NAME to the environment, never to a flat slot, and the compiler itself declines the same shape (`...is not eligible for dynamic unified bytecode assignment references.`). Admitting requires a new slot-reference-store lowering (push slot reference, store-to-slot, leave value on stack) — compiler+VM foundation work, not a clean admit. The SAME assignment written as a top-level STATEMENT (value discarded, e.g. `x = 5;`) uses `AssignmentSlotInstruction` → `StoreSlot` and routes | Existing execution-plan route | Statement/control-flow ownership lane | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~ExpressionProgramCoverageMapTests&FullyQualifiedName~UnifiedBytecodeExpansionContract_ListsRequiredHeadingsAndCurrentEnums"` |
...
- `_homeObject` is set for all class methods and object-literal methods defined
  ...
  such as `fn(box?.value?.nested)` and `fn(box?.child?.value)`; computed
  optional continuation call arguments still decline with
  `OptionalChainDependency` (see the call-argument narrative below). A29: the
...
- Current executable call support covers
  ...
  hop short-circuits the whole chain to `undefined`; computed optional
  continuations such as `box?.prop?.[key]` still decline with
  `OptionalChainDependency`), optional-named-then-plain-computed read
