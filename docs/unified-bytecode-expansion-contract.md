...
| `ArgumentsObjectDependency` | Activation descriptor and expression scan for real `arguments` object call targets; implicit `arguments` reads/updates materialize the existing arguments object and `typeof arguments` uses the same bounded object route, while parameter/lexical `arguments` reads, `typeof`, updates, and call targets route as activation slots | Existing sync IR / arguments-object route | Arguments object lane | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&FullyQualifiedName~Evaluate_UpdateImplicitArgumentsObject_AcceptsDynamicIdentifierUpdate"` |
