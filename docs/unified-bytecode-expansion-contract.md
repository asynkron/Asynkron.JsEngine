...
- Public class methods with active or captured private-name scopes are no longer
  ...
  `PrivateFieldIn` and direct private named reads/writes can execute through
  `GetNamedProperty` / `SetNamedProperty`. Private member updates/deletes and
  private call-target preparation still decline as `PrivateFieldDependency`.
