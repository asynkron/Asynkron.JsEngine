...
- Completing the decline-burndown plan did not make unified bytecode the only
  ...
  execution paths.
- E4/E5 owner inventories are classification evidence, not closure claims.
  Remaining ordinary work is the non-dynamic A/B/C parity and retirement surface
  named in the checklist and proof manifest. Terminal dynamic residue stays
  separate: multi-argument, spread, runtime-source, or declaration-injecting
  direct eval; awaited `with` object evaluation; retained live `with` scopes
  outside the VM current-environment lane; eval-injected runtime bindings; and
