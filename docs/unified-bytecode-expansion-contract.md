...
|---|---|---|---|---|
| `prototype-guard:Binary` | `Binary` opcodes are admitted only for `+`, `-`, `*`, `/`, `%`, `==`, `<`, `<=`, `>`, and `>=`; all other binary operators decline as `PrototypeOnlyBinaryOpcode` | Existing expression route | Binary operator widening lane | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&FullyQualifiedName~Evaluate_PropertyReadAdjacentFamilies_DeclineWithExplicitCodes"` |
