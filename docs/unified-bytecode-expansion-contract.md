...
- The remaining coarse P0.4 leaves are decomposed under
  ...
  static-block-only class-expression subset whose static blocks do not create
...
| Standalone expression and binding bridges | `TypedAstEvaluator.ExpressionPrograms`, `TypedAstEvaluator.BindingTargetPrograms`, and `ExecutionPlanRunner.Core` helper entrypoints | `EvaluateStandaloneExpressionProgram` is deleted. `EvaluateLoweredExpressionProgram` remains an E4 bridge surface, but it now compiles standalone `ExpressionProgram` payloads through `UnifiedBytecodeCompiler.TryCompileStandaloneExpressionProgram` and executes `UnifiedBytecodeVirtualMachine` directly. Binding-target programs still use the bounded runner bridge through `ApplyStandaloneBindingTargetProgram`. The profiler-only expression-program loop is also gone: `ProfileRunner` compiles synthetic profile cases to standalone unified bytecode and executes the unified VM directly, while `ProfileEvaluateExpressionProgramLoop` is source-gated as absent. | `rtk rg -n "EvaluateStandaloneExpressionProgram|EvaluateLoweredExpressionProgram|ApplyStandaloneBindingTargetProgram" src/Asynkron.JsEngine/Ast`; `rtk rg -n "ProfileEvaluateExpressionProgramLoop" src/Asynkron.JsEngine/Ast` |
