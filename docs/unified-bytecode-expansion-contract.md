...
- Public class methods with active or captured private-name scopes are no longer
  ...
  `PrivateFieldIn` and direct private named reads can execute through
  `GetNamedProperty`. Private member writes/updates/deletes and private
  call-target preparation still decline as `PrivateFieldDependency`.
