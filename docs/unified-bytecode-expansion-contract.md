...
- `A51f3:DynamicIdentifierExpressionLoop` - narrowed. Zero-depth
  catch/finally-region free identifier reads and `typeof` free identifiers now
  enable the ordinary dynamic-name route through `LoadDynamicIdentifier` /
  `TypeOfDynamicIdentifier`, matching the existing free-call-target
  exception-region scan. Stores, updates, assignment references, deletes,
