...
| `A51b` | Activation-slot metadata, slot-layout, declaration / assignment / update storage targets; manifest `A51b-*stays-open` source-presence rows pin the compiler and eligibility guards while the row remains open | Existing execution-plan runner | both |
