...
| `A51h` | General array/object/template literal-span shapes and restricted literal/span helper diagnostics for unsupported simple, logical-control, and conditional consequent/alternate operands; manifest `A51h-*stays-open` rows pin the non-spread/non-computed-key source anchors | Existing expression-program literal evaluation | both |
