...
- Current executable call support covers
  ...
  simple or simple-binary array/object/template literal values or elements
  (`{items: [a, b]}`, `[{x}]`, `{total: a + b}`,
  ``{label: `hello ${name}`}``) and object literal spread entries
