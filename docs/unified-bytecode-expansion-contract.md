...
| `A51i` | Computed/optional/private property-read and receiver-boundary shapes; read-only private receiver-prefix chains such as `receiver.#child.value` and `receiver.#child[key]`, private receiver-prefix named calls such as `receiver.#child.value()`, plus single-hop optional private reads such as `receiver?.#field`, are now sync-admitted through owned property-read/call-target opcodes; unsupported property-read base diagnostics stay here | Existing expression-program property evaluation for remaining private-adjacent property reads | both |
...
- `A51f5:PrivateNeighborExpressionLoop` - narrowed. Plain private named reads
  ...
  minimal receiver-prefix named write `receiver.#child.value = value` routes by
  composing a single non-optional private receiver read with a public named
  terminal `SetNamedProperty`. Private named delete defense/manual-plan
  diagnostics, chained optional-private receiver-neighbor reads, private
  receiver-neighbor compound/logical/update/computed/private-terminal writes,
