...
| `A51i` | Computed/optional/private property-read and receiver-boundary shapes; read-only private receiver-prefix chains such as `receiver.#child.value` and `receiver.#child[key]`, plus single-hop optional private reads such as `receiver?.#field`, are now sync-admitted through owned property-read opcodes; unsupported property-read base diagnostics stay here | Existing expression-program property evaluation for remaining private-adjacent property reads | both |
...
- `A51f5:PrivateNeighborExpressionLoop` - narrowed. Plain private named reads
  ...
  optional private reads route through `GetNamedPropertyOptional`. Private named
  delete defense/manual-plan diagnostics, chained optional-private
  receiver-neighbor reads, private receiver-neighbor mutation or call shapes,
