...
| `PropertyWriteDependency` | Property writes and compound/logical property writes outside the admitted direct property-write shapes, simple nested named receiver assignment shape, and nested named compound-write shape | Existing sync IR property-write route | Property write widening lane | `rtk dotnet test tests/Asynkron.JsEngine.Tests --filter "FullyQualifiedName~UnifiedBytecodeProductionEligibilityTests&FullyQualifiedName~Evaluate_LogicalAndAssignment_UnsupportedShapes_DeclineWithExplicitCodes"` |
...
- `_homeObject` is set for all class methods and object-literal methods defined
  ...
  target). Simple nested named receiver assignments, compound writes, and updates
  (`box.child.value = y`, `box.child.value += y`, `box.child.value++`) are admitted through
