...
- **Root cause**: Fast-path invocation (`InvokeSimpleFast*` / `InvokeSimpleFastCore*` in `src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncFunctionInvoker.cs`) never calls `EvaluationContext.EnterPrivateNameScope(s)`, so the private-name stack stays empty during fast-path execution.
...
- **Evidence**:
  - Slow path pushes scopes:
    - `TypedAstEvaluator.SyncFunctionInvoker.InvokeWithContextSlow` uses:
      ```
