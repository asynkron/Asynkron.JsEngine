...
- Fast-path invocations in `SyncFunctionInvoker` (`InvokeSimpleFast*`/`InvokeSimpleFastCore*`) never enter `EvaluationContext.EnterPrivateNameScope` or `EnterPrivateNameScopes`. The slow path does, so only fast path runs with an empty private-scope stack.
...
- Slow path pushes private scopes:
  - `src/Asynkron.JsEngine/Ast/TypedAstEvaluator.SyncFunctionInvoker.cs` (`InvokeWithContextSlow`): enters `_capturedPrivateNameScopes` and `PrivateNameScope`.
- Fast path does not push scopes:
  - `...InvokeSimpleFastCore*` in `SyncFunctionInvoker.cs`: no `EnterPrivateNameScope`/`EnterPrivateNameScopes`; only a debug guard that throws if scopes are present.
