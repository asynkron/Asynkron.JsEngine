...
- Quarantined expression operands that only need a value execute cached lowered
  `ExpressionProgram` payloads through `LoweredExpressionProgramCache` and
  `UnifiedBytecodeExpressionProgramExecutor.ExecuteStandalone`. They must not
  fall back to raw AST expression evaluation on compile failure.
  `EvaluateDynamicExpressionProgram` and
  `UnifiedBytecodeExpressionProgramExecutor.ExecuteDynamic` are tombstoned.
