...
public static partial class TypedAstEvaluator
{
    private sealed partial class ExecutionPlanRunner
    {
                    ...
                    out JsValue returnValue)
        {
            ...
            if (yieldStarState.State is null)
            {
                ...
                var yieldStarIterableValue = instruction.AwaitedProgram is { } awaitedProgram
                    ? runner.EvaluateAwaitInGenerator(instruction.AwaitStateKey!, awaitedProgram, environment, context)
                    : runner.EvaluateExpressionProgram(instruction.IterableProgram!.Value, environment, context);
        ...
        [MethodImpl(JsEngineConstants.Inlining)]
        private static InstructionResult HandleSuspendingYieldStar(
                    ExecutionPlanRunner runner,
                    ExecutionInstruction instr,
                    ref JsEnvironment environment,
                    EvaluationContext context,
                    out JsValue returnValue)
        {
            var instruction = Unsafe.As<SuspendingYieldStarInstruction>(instr);
            var currentIndex = runner._programCounter;
            if (!TryGetSymbolValueJsValue(environment, instruction.StateSlotSymbol,
                    out var stateValue) ||
                !stateValue.TryGetObject<YieldStarState>(out var yieldStarState))
            ...
            {
                runner._realmState.Logger?.LogInformation("YieldStar: Creating new DelegatedState");
                var yieldStarIterableValue = instruction.IterableExpression.EvaluateExpression(environment, context);
                if (runner._isAsync && runner.TryHandlePendingAwait(context, out var pendingYieldStarResult, environment))
                {
                    returnValue = pendingYieldStarResult;
                    return InstructionResult.Return;
