...
{
    private static void EvaluateVariableDeclarator(this VariableKind kind, VariableDeclarator declarator,
        JsEnvironment environment, EvaluationContext context)
    {
        ...
        using var functionNameHint = targetIdentifier is not null &&
                                     declarator.Initializer?.IsAnonymousFunctionDefinitionNode() == true
            ? context.EnterFunctionNameHint(targetIdentifier.Name)
            : null;

        var valueJs = declarator.Initializer is null
            ? JsValue.Undefined
            : ((IAstCacheable<LoweredExpressionProgramCache>)declarator)
                .GetOrCreateCache()
                .Execute(environment, context, "Variable initializer");
        ...
        var allowNameInference = declarator.Initializer?.IsAnonymousFunctionDefinitionNode() == true;
