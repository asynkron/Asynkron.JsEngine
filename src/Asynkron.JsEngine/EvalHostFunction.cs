    ...
    IJsObjectLike, IPropertyDefinitionHost, IExtensibilityControl, IPrototypeAccessorProvider, ICallableMetadata
{
        ...
        bool inClassFieldInitializer)
    {
        ...
        var invalidPrivateName = PrivateNameValidator.FindInvalidPrivateName(program.Body, evalPrivateNameScopes);
        ...
        if (!isStrictEval)
        {
            ...
            foreach (var name in varDeclaredNames)
            {
                ...
                // For arrows, this only applies when the parameter environment itself
                // declares an `arguments` binding (for example a parameter named arguments).
    ...
    private EvalProgramCacheValue GetOrParseProgram(string code, bool forceStrict, bool hasStrictCaller)
    {
        ...
        if (lastEntry?.Key.Equals(key) == true)
        {
