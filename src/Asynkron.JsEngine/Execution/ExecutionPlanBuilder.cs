...
internal sealed class ExecutionPlanBuilder
{
    ...
    {
        // Step 1: Collect all unique user variable symbols from instructions
        var userVariables = new HashSet<Symbol>(ReferenceEqualityComparer<Symbol>.Instance);
        foreach (var instruction in _instructions)
        {
            CollectIdentifiersFromInstruction(instruction, userVariables);
        }

        // Step 2: Build a map from symbol to slot index
        var symbolToSlot = new Dictionary<Symbol, int>(userVariables.Count, ReferenceEqualityComparer<Symbol>.Instance);
        foreach (var symbol in userVariables)
        {
            // Allocate a slot for this user variable
            var slotIndex = AllocateSlot(symbol);
            symbolToSlot[symbol] = slotIndex;
        }

        // Step 3: Update all instructions to use the new slot information
        if (symbolToSlot.Count > 0)
        {
            for (var i = 0; i < _instructions.Count; i++)
            {
                _instructions[i] = UpdateInstructionWithSlots(_instructions[i], symbolToSlot);
            }
        }
    }

    /// <summary>
    /// Recursively collects all IdentifierExpression symbols from an instruction.
    /// </summary>
    private static void CollectIdentifiersFromInstruction(ExecutionInstruction instruction, HashSet<Symbol> symbols)
    {
        switch (instruction)
        {
            case StatementInstruction stmt:
                CollectIdentifiersFromStatement(stmt.Statement, symbols);
                break;
            case ExpressionInstruction expr:
                CollectIdentifiersFromExpression(expr.Expression, symbols);
                break;
            case EvaluateAndDiscardInstruction eval:
                CollectIdentifiersFromExpression(eval.Expression, symbols);
                break;
            case YieldInstruction yield when yield.YieldExpression is not null:
                CollectIdentifiersFromExpression(yield.YieldExpression, symbols);
                break;
            case ReturnInstruction ret when ret.ReturnExpression is not null:
                CollectIdentifiersFromExpression(ret.ReturnExpression, symbols);
                break;
            case ThrowInstruction thr:
                CollectIdentifiersFromExpression(thr.Expression, symbols);
                break;
            case PushEnvironmentInstruction push:
                if (push.SlotMap is not null)
                {
                    foreach (var sym in push.SlotMap.Keys)
                    {
                        symbols.Add(sym);
                    }
                }
    ...
    private static void CollectIdentifiersFromStatement(StatementNode statement, HashSet<Symbol> symbols)
    {
        // NOTE: We need to collect from ALL statement types to ensure every identifier gets a slot,
        // even though we only update simple statements (complex ones are wrapped in StatementInstruction).
        // This is because identifiers can appear deeply nested in the AST.
        switch (statement)
        {
            case ExpressionStatement exprStmt:
                CollectIdentifiersFromExpression(exprStmt.Expression, symbols);
                break;
            case BlockStatement block:
                foreach (var stmt in block.Statements)
                {
                    CollectIdentifiersFromStatement(stmt, symbols);
                }
                break;
            case ReturnStatement ret when ret.Expression is not null:
                CollectIdentifiersFromExpression(ret.Expression, symbols);
                break;
            ...
            case VariableDeclaration varDecl:
                foreach (var decl in varDecl.Declarators)
                {
                    ...
                    {
                        CollectIdentifiersFromExpression(decl.Initializer, symbols);
                    }
                }
                break;
            // Explicitly handle loop and conditional statements to collect all identifiers
            case WhileStatement whileStmt:
                CollectIdentifiersFromExpression(whileStmt.Condition, symbols);
                CollectIdentifiersFromStatement(whileStmt.Body, symbols);
                break;
            case DoWhileStatement doWhileStmt:
                CollectIdentifiersFromStatement(doWhileStmt.Body, symbols);
                CollectIdentifiersFromExpression(doWhileStmt.Condition, symbols);
                break;
            case IfStatement ifStmt:
                CollectIdentifiersFromExpression(ifStmt.Condition, symbols);
                CollectIdentifiersFromStatement(ifStmt.Then, symbols);
                if (ifStmt.Else is not null)
                {
                    CollectIdentifiersFromStatement(ifStmt.Else, symbols);
                }
                break;
            // For other complex statements, use reflection as fallback
            case ForStatement or ForEachStatement or TryStatement or SwitchStatement:
                CollectFromAnyStatement(statement, symbols);
                break;
        }
    }

    private static void CollectFromAnyStatement(StatementNode statement, HashSet<Symbol> symbols)
    {
        // Reflection-based fallback for complex statement types
        var type = statement.GetType();
        foreach (var prop in type.GetProperties())
        {
            var value = prop.GetValue(statement);
            if (value is ExpressionNode expr)
            {
                CollectIdentifiersFromExpression(expr, symbols);
            }
            else if (value is StatementNode stmt)
            {
                CollectIdentifiersFromStatement(stmt, symbols);
            }
            else if (value is System.Collections.IEnumerable enumerable and not string)
            {
                ...
                {
                    if (item is ExpressionNode exprItem)
                    {
                        CollectIdentifiersFromExpression(exprItem, symbols);
                    }
                    else if (item is StatementNode stmtItem)
                    {
                        CollectIdentifiersFromStatement(stmtItem, symbols);
                    }
                }
            }
        }
    ...
    private static void CollectIdentifiersFromExpression(ExpressionNode expression, HashSet<Symbol> symbols)
    {
        while (true)
        {
            switch (expression)
            {
                ...
                case BinaryExpression binary:
                    CollectIdentifiersFromExpression(binary.Left, symbols);
                ...
                case AssignmentExpression assignment:
                    ...
                    symbols.Add(assignment.Target);
                    expression = assignment.Value;
                    continue;
                case PropertyAssignmentExpression propAssign:
                    CollectIdentifiersFromExpression(propAssign.Target, symbols);
                    CollectIdentifiersFromExpression(propAssign.Property, symbols);
                    expression = propAssign.Value;
                    continue;
                case IndexAssignmentExpression indexAssign:
                    CollectIdentifiersFromExpression(indexAssign.Target, symbols);
                    CollectIdentifiersFromExpression(indexAssign.Index, symbols);
                    expression = indexAssign.Value;
                    continue;
                case DestructuringAssignmentExpression destructAssign:
                    expression = destructAssign.Value;
                    continue;
                case CallExpression call:
                    CollectIdentifiersFromExpression(call.Callee, symbols);
                    foreach (var arg in call.Arguments)
                    {
                        if (arg.Expression is not null)
                        {
                            CollectIdentifiersFromExpression(arg.Expression, symbols);
                        }
                    }

                    break;
                case MemberExpression member:
                    CollectIdentifiersFromExpression(member.Target, symbols);
                    if (!member.IsComputed)
                    {
                        // Non-computed property is a literal, not an identifier
                    }
                    else
                    {
                ...
                case ConditionalExpression conditional:
                    CollectIdentifiersFromExpression(conditional.Test, symbols);
                    CollectIdentifiersFromExpression(conditional.Consequent, symbols);
                    expression = conditional.Alternate;
                    continue;
                case SequenceExpression sequence:
                    CollectIdentifiersFromExpression(sequence.Left, symbols);
                ...
                case ArrayExpression array:
                    foreach (var element in array.Elements)
                    {
                        ...
                        {
                            CollectIdentifiersFromExpression(element.Expression, symbols);
                        }
                    }
                ...
                case ObjectExpression obj:
                    foreach (var member in obj.Members)
                    {
                        ...
                        {
                            CollectIdentifiersFromExpression(keyExpr, symbols);
                        }

                        if (member.Value is not null)
                        {
                            CollectIdentifiersFromExpression(member.Value, symbols);
                        }
                    }
    ...
    /// <summary>
    /// Updates an instruction to use slot information for identifiers.
    /// </summary>
    private static ExecutionInstruction UpdateInstructionWithSlots(
        ExecutionInstruction instruction,
        Dictionary<Symbol, int> symbolToSlot)
    {
        return instruction switch
        {
            StatementInstruction stmt => stmt with
            {
                Statement = UpdateStatementWithSlots(stmt.Statement, symbolToSlot)
            },
            ExpressionInstruction expr => expr with
            {
                Expression = UpdateExpressionWithSlots(expr.Expression, symbolToSlot)
            },
            EvaluateAndDiscardInstruction eval => eval with
            {
                Expression = UpdateExpressionWithSlots(eval.Expression, symbolToSlot)
            },
            YieldInstruction yield when yield.YieldExpression is not null => yield with
            {
                YieldExpression = UpdateExpressionWithSlots(yield.YieldExpression, symbolToSlot)
            },
            ReturnInstruction ret when ret.ReturnExpression is not null => ret with
            {
                ReturnExpression = UpdateExpressionWithSlots(ret.ReturnExpression, symbolToSlot)
            },
            ThrowInstruction thr => thr with
            {
                Expression = UpdateExpressionWithSlots(thr.Expression, symbolToSlot)
            },
            _ => instruction
        };
    }

    private static StatementNode UpdateStatementWithSlots(
        ...
        Dictionary<Symbol, int> symbolToSlot)
    {
        return statement switch
        {
            ...
            {
                Expression = UpdateExpressionWithSlots(exprStmt.Expression, symbolToSlot)
            },
            BlockStatement block => block with
            {
                Statements = [..block.Statements.Select(s => UpdateStatementWithSlots(s, symbolToSlot))]
            },
            ReturnStatement ret when ret.Expression is not null => ret with
            {
                Expression = UpdateExpressionWithSlots(ret.Expression, symbolToSlot)
            },
            ThrowStatement thr => thr with
            {
                Expression = UpdateExpressionWithSlots(thr.Expression, symbolToSlot)
            },
            // Complex statements are wrapped in StatementInstruction, so we don't need to update them
            _ => statement
        };
    }

    private static ExpressionNode UpdateExpressionWithSlots(
        ExpressionNode expression,
        Dictionary<Symbol, int> symbolToSlot)
    {
        return expression switch
        {
            IdentifierExpression identifier when symbolToSlot.TryGetValue(identifier.Name, out var slotIndex) =>
                identifier with
                {
                    ScopeId = 0, // Execution environment always uses ScopeId=0
                    SlotIndex = slotIndex
                },
            BinaryExpression binary => binary with
            {
                Left = UpdateExpressionWithSlots(binary.Left, symbolToSlot),
                Right = UpdateExpressionWithSlots(binary.Right, symbolToSlot)
            },
            UnaryExpression unary => unary with
            {
                Operand = UpdateExpressionWithSlots(unary.Operand, symbolToSlot)
            },
            AssignmentExpression assignment => assignment with
            {
                // Target is a Symbol - update slot info for it
                ScopeDepth = symbolToSlot.ContainsKey(assignment.Target) ? 0 : assignment.ScopeDepth,
                SlotIndex = symbolToSlot.TryGetValue(assignment.Target, out var targetSlot) ? targetSlot : assignment.SlotIndex,
                Value = UpdateExpressionWithSlots(assignment.Value, symbolToSlot)
            },
            PropertyAssignmentExpression propAssign => propAssign with
            {
                Target = UpdateExpressionWithSlots(propAssign.Target, symbolToSlot),
                Property = UpdateExpressionWithSlots(propAssign.Property, symbolToSlot),
                Value = UpdateExpressionWithSlots(propAssign.Value, symbolToSlot)
            },
            IndexAssignmentExpression indexAssign => indexAssign with
            {
                Target = UpdateExpressionWithSlots(indexAssign.Target, symbolToSlot),
                Index = UpdateExpressionWithSlots(indexAssign.Index, symbolToSlot),
                Value = UpdateExpressionWithSlots(indexAssign.Value, symbolToSlot)
            },
            DestructuringAssignmentExpression destructAssign => destructAssign with
            {
                Value = UpdateExpressionWithSlots(destructAssign.Value, symbolToSlot)
            },
            CallExpression call => call with
            {
                Callee = UpdateExpressionWithSlots(call.Callee, symbolToSlot),
                Arguments = [..call.Arguments.Select(a => a.Expression is not null
                    ? a with { Expression = UpdateExpressionWithSlots(a.Expression, symbolToSlot) }
                    : a)]
            },
            MemberExpression member => member with
            {
                Target = UpdateExpressionWithSlots(member.Target, symbolToSlot),
                Property = member.IsComputed
                    ? UpdateExpressionWithSlots(member.Property, symbolToSlot)
                    : member.Property
            },
            ConditionalExpression conditional => conditional with
            {
                Test = UpdateExpressionWithSlots(conditional.Test, symbolToSlot),
                Consequent = UpdateExpressionWithSlots(conditional.Consequent, symbolToSlot),
                Alternate = UpdateExpressionWithSlots(conditional.Alternate, symbolToSlot)
            },
            SequenceExpression sequence => sequence with
            {
                Left = UpdateExpressionWithSlots(sequence.Left, symbolToSlot),
                Right = UpdateExpressionWithSlots(sequence.Right, symbolToSlot)
            },
            ArrayExpression array => array with
            {
                Elements = [..array.Elements.Select(e => e.Expression is not null
                    ? e with { Expression = UpdateExpressionWithSlots(e.Expression, symbolToSlot) }
                    : e)]
            },
            ObjectExpression obj => obj with
            {
                Members = [..obj.Members.Select(m => m with
                {
                    Key = m.Key is ExpressionNode keyExpr
                        ? UpdateExpressionWithSlots(keyExpr, symbolToSlot)
                        : m.Key,
                    Value = m.Value is not null
                        ? UpdateExpressionWithSlots(m.Value, symbolToSlot)
                        : null
                })]
            },
            YieldExpression yield when yield.Expression is not null => yield with
            {
                Expression = UpdateExpressionWithSlots(yield.Expression, symbolToSlot)
            },
            AwaitExpression await => await with
            {
                Expression = UpdateExpressionWithSlots(await.Expression, symbolToSlot)
            },
            _ => expression
        };
    }
