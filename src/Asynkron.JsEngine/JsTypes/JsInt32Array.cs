    ...
    : TypedArrayBase(buffer, byteOffset, length, BYTES_PER_ELEMENT, isLengthTracking)
{
    ...
    public override string TypedArrayName => "Int32Array";

    public static JsInt32Array FromLength(int length, RealmState? realmState = null)
    {
        var buffer = new JsArrayBuffer(length * BYTES_PER_ELEMENT, null, realmState);
        return new JsInt32Array(buffer, 0, length);
    }

    public static JsInt32Array FromArray(JsArray array, RealmState? realmState = null)
    {
    ...
    {
        return new JsInt32Array(buffer, byteOffset, length);
    }
}
