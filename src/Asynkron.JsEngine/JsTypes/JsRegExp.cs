...
public sealed class JsRegExp
{
    ...
    private JsArray CreateMatchArray(Match match, string input)
    {
        ...
        if (HasIndices)
        {
            ...
            result.DefineProperty("indices",
                new PropertyDescriptor
                {
                    Value = JsValue.FromJsArray(indices), Writable = true, Enumerable = true, Configurable = true
                });
        }
    ...
    private JsObject? BuildGroupsObject(Match match, JsValue[] captureValues)
    {
        ...
        foreach (var name in match.Groups.Keys)
        {
            ...
            if (groups is null)
            {
                // Per spec: groups = ObjectCreate(null)
                groups = new JsObject();
                groups.SetPrototype(null);
            }

            // Per spec: CreateDataProperty
            groups.DefineProperty(name,
                new PropertyDescriptor
                {
                    Value = captureValues[groupNumber], Writable = true, Enumerable = true, Configurable = true
                });
        }
    ...
    private JsArray BuildIndicesArray(Match match)
    {
        ...
        // Per spec: CreateDataProperty(indices, "groups", groups)
        indices.DefineProperty("groups",
            new PropertyDescriptor
            {
                Value = groups is null ? JsValue.Undefined : JsValue.FromJsObject(groups),
                Writable = true, Enumerable = true, Configurable = true
    ...
    private static JsObject? BuildIndicesGroupsObject(Match match, Regex regex, JsValue[] indexValues)
    {
        ...
        foreach (var name in match.Groups.Keys)
        {
            ...
            groups.DefineProperty(name,
                new PropertyDescriptor
                {
                    Value = indexValues[groupNumber], Writable = true, Enumerable = true, Configurable = true
                });
        }
