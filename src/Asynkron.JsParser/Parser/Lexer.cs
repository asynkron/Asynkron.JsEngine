    ...
    private void ReadNumber()
    {
        ...
        if (_source[_start] == '0' && _current < _source.Length)
        {
            ...
            if (next is 'x' or 'X')
            {
                ...
                if (Peek() == 'n')
                {
                    ...
                    {
                        Advance(); // consume 'n'
                        var value = ParseIntegerLiteral(hexSpan, 16, hexDigits.HasSeparator);
            ...
            if (next is 'o' or 'O')
            {
                ...
                if (Peek() == 'n')
                {
                    ...
                    {
                        Advance(); // consume 'n'
                        var value = ParseIntegerLiteral(octalSpan, 8, octalDigits.HasSeparator);
                        AddToken(TokenType.BigInt, value);
                        return;
                    }
                }
            ...
            if (next is 'b' or 'B')
            {
                ...
                if (Peek() == 'n')
                {
                    ...
                    {
                        Advance(); // consume 'n'
                        var value = ParseIntegerLiteral(binarySpan, 2, binaryDigits.HasSeparator);
                        AddToken(TokenType.BigInt, value);
                        return;
                    }
                }
        ...
        if (!hasDecimal && Peek() == 'n')
        {
            ...
            if (isEndOrNonAlphaNum)
            {
                ...
                var value = ParseIntegerLiteral(digitsSpan, 10, hasSeparator);
                AddToken(TokenType.BigInt, value);
                return;
            }
        }
