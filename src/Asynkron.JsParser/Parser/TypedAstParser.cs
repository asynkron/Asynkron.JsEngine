...
public sealed class TypedAstParser(
    ...
    IParserOptions? options = null)
{
    private readonly IParserOptions _options = options ?? ParserOptions.Default;
        ...
        IParserOptions options)
    {
            ...
            bool allowInitializerless = false)
        {
            ...
            do
            {
                ...
                else if (!allowInitializerless &&
                         (isUsingDeclaration || kind == VariableKind.Const || requiresInitializer))
                {
        ...
        private ObjectBinding ParseObjectBindingPattern(Token startToken)
        {
            ...
            while (!Check(TokenType.RightBrace))
            {
                ...
                {
                    var start = Previous();
                    nameExpression = ParseAssignment(false);
        ...
        private (string Name, bool CanUseShorthand, Token Token) ParseBindingPropertyName()
        {
            ...
            if (Match(TokenType.BigInt))
            {
                ...
                var value = token.Literal is System.Numerics.BigInteger bigInt
                    ? bigInt.ToString(CultureInfo.InvariantCulture)
        ...
        private StatementNode ParseForStatement(Token forToken)
        {
            ...
            if (isForEach)
            {
                ...
                var assignment =
                    CreateInitializerAssignment(initializerDeclaration.Declarators[0].Target, varInitializer);
        ...
        private ExpressionNode ParseUnary()
        {
            ...
            {
                return new UnaryExpression(CreateSourceReference(Previous()), UnaryOperator.Increment, ParseUnary(),
                    true);
            }

            if (Match(TokenType.MinusMinus))
            {
                return new UnaryExpression(CreateSourceReference(Previous()), UnaryOperator.Decrement, ParseUnary(),
                    true);
            }

            if (Match(TokenType.Bang))
            {
                return new UnaryExpression(CreateSourceReference(Previous()), UnaryOperator.LogicalNot, ParseUnary(),
                    true);
            }
            ...
            {
                return new UnaryExpression(CreateSourceReference(Previous()), UnaryOperator.BitwiseNot, ParseUnary(),
                    true);
            }
        ...
        private ExpressionNode ParsePrimary(bool allowCallSuffix = true)
        {
            ...
            if (Match(TokenType.Number))
            {
                ...
                expr = new LiteralExpression(CreateSourceReference(token), new LiteralValue((double)token.Literal!));
                return ApplyCallSuffix(expr, allowCallSuffix);
            }
            ...
            if (Match(TokenType.BigInt))
            {
                var token = Previous();
                EnsureNumericLiteralAllowed();
                expr = new LiteralExpression(CreateSourceReference(token), new LiteralValue((System.Numerics.BigInteger)token.Literal!));
                return ApplyCallSuffix(expr, allowCallSuffix);
            }
            ...
            if (Match(TokenType.String))
            {
                ...
                expr = new LiteralExpression(CreateSourceReference(token), new LiteralValue(literalValue));
                return ApplyCallSuffix(expr, allowCallSuffix);
            }
            ...
            {
                expr = new LiteralExpression(CreateSourceReference(Previous()), LiteralValue.True);
                return ApplyCallSuffix(expr, allowCallSuffix);
            }

            if (Match(TokenType.False))
            {
                expr = new LiteralExpression(CreateSourceReference(Previous()), LiteralValue.False);
                return ApplyCallSuffix(expr, allowCallSuffix);
            }

            if (Match(TokenType.Null))
            {
                expr = new LiteralExpression(CreateSourceReference(Previous()), LiteralValue.Null);
                return ApplyCallSuffix(expr, allowCallSuffix);
            }
        ...
        private ExpressionNode ParseArrayLiteral
        {
            get
            {
                ...
                while (!Check(TokenType.RightBracket))
                {
                    ...
                    try
                    {
                        var expr = ParseExpression(false, false);
        ...
        private ExpressionNode ParseObjectLiteral()
        {
            ...
            while (!Check(TokenType.RightBrace))
            {
                ...
                if (Match(TokenType.DotDotDot))
                {
                    var spreadExpr = ParseExpression(false, false);
                ...
                else if (Match(TokenType.Colon))
                {
                    ...
                    try
                    {
                        value = ParseExpression(false, false);
                    }
                    finally
                    {
                        _allowInExpressions = previousAllowIn;
                ...
                else
                {
                    ...
                    if (Match(TokenType.Equal))
                    {
                        ...
                        try
                        {
                            var initializer = ParseAssignment(false);
        ...
        private ArrayExpression BuildTemplateStringsArrayWithNullable(IReadOnlyList<string?> values,
            Token templateToken)
        {
            var elements = ImmutableArray.CreateBuilder<ArrayElement>(values.Count);
            foreach (var text in values)
            {
                ...
                var jsValue = text is null ? LiteralValue.Undefined : new LiteralValue(text);
        ...
        private string GetPropertyNameValue(Token token)
        {
            return token.Type switch
            {
                ...
                {
                    System.Numerics.BigInteger bigInt => bigInt.ToString(CultureInfo.InvariantCulture),
