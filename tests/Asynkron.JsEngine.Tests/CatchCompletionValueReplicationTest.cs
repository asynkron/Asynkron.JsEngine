...
public sealed class CatchCompletionValueReplicationTest(ITestOutputHelper output)
{
    ...
    {
        var logger = new TestLogger(output, "CptnCatch", minLogLevel: LogLevel.Debug);
        ...
        output.WriteLine($"Empty catch result: {result1} (expected: undefined)");
        ...
        output.WriteLine($"Catch with value result: {result2} (expected: 3)");
    ...
    {
        var logger = new TestLogger(output, "CptnTry", minLogLevel: LogLevel.Debug);
        await using var engine = new JsEngine(new JsEngineOptions { DebugMode = true, Logger = logger });

        // Test 1: Empty try block should return undefined
        var result1 = await engine.Evaluate("eval('1; try { } catch (e) { }')");
        output.WriteLine($"Empty try result: {result1} (expected: undefined)");
        Assert.True(result1 is null || ReferenceEquals(result1, Asynkron.JsEngine.Ast.Symbol.Undefined),
        ...
        output.WriteLine($"Try with value result: {result2} (expected: 3)");
        Assert.Equal(3.0, result2);

        // Test 3: Empty try, catch with value - catch not executed
        var result3 = await engine.Evaluate("eval('4; try { } catch (e) { 5; }')");
        output.WriteLine($"Empty try (catch not executed) result: {result3} (expected: undefined)");
        ...
        output.WriteLine($"Try with value (catch not executed) result: {result4} (expected: 7)");
    ...
    {
        var logger = new TestLogger(output, "CptnIf", minLogLevel: LogLevel.Debug);
        await using var engine = new JsEngine(new JsEngineOptions { DebugMode = true, Logger = logger });

        // Test 1: Empty if and else - should return undefined
        var result1 = await engine.Evaluate("eval('1; if (false) { } else { }')");
        output.WriteLine($"Empty else result: {result1} (expected: undefined)");
        Assert.True(result1 is null || ReferenceEquals(result1, Asynkron.JsEngine.Ast.Symbol.Undefined),
        ...
        output.WriteLine($"Else with value result: {result2} (expected: 3)");
        Assert.Equal(3.0, result2);

        // Test 3: If with value, empty else
        var result3 = await engine.Evaluate("eval('4; if (false) { 5; } else { }')");
        output.WriteLine($"Empty else (if has value) result: {result3} (expected: undefined)");
        Assert.True(result3 is null || ReferenceEquals(result3, Asynkron.JsEngine.Ast.Symbol.Undefined),
        ...
        output.WriteLine($"Else with value (both have values) result: {result4} (expected: 8)");
    ...
    {
        var logger = new TestLogger(output, "ContinueFromCatch", minLogLevel: LogLevel.Debug);
        await using var engine = new JsEngine(new JsEngineOptions { DebugMode = true, Logger = logger });
        ...
        output.WriteLine($"Continue from catch result: {result} (expected: 3)");
    ...
    public async Task Replicate_BreakFromFinally()
    {
        ...
        output.WriteLine($"Break from finally result: {result} (expected: 1)");
    ...
    {
        var logger = new TestLogger(output, "ForCptn", minLogLevel: LogLevel.Debug);
        await using var engine = new JsEngine(new JsEngineOptions { DebugMode = true, Logger = logger });
        ...
        output.WriteLine($"For loop completion result: {result} (expected: 2)");
    ...
    {
        var logger = new TestLogger(output, "SwitchCptn", minLogLevel: LogLevel.Debug);
        await using var engine = new JsEngine(new JsEngineOptions { DebugMode = true, Logger = logger });
        ...
        output.WriteLine($"Switch abrupt empty result: {result} (expected: undefined)");
