...
public sealed class IrLoopEnvironmentTests(ITestOutputHelper output) : InternalTestBase(output)
{
    ...
    public async Task AsyncAssignmentExpression_UsesAwaitedAssignmentInstruction()
    {
        ...
        Assert.Contains(
            awaitedProgram.Operations.Select(static op => op.ToLegacyExpressionOp()).OfType<LoadIdentifierExpressionOp>(),
    ...
    public async Task GeneratorWithYieldingObject_ReusesEnterWithInstruction()
    {
        ...
        Assert.Contains(
            objectProgram.Operations.Select(static op => op.ToLegacyExpressionOp()).OfType<LoadIdentifierExpressionOp>(),
