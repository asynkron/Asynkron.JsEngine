...
public sealed class UnifiedBytecodeProductionInvocationTests(ITestOutputHelper output) : InternalTestBase(output)
{
    ...
    public async Task IdentifierCallWithOptionalNamedPropertyReadArgument_UsesUnifiedBytecodeProductionFastPath()
    {
        ...
        var result = await engine.Evaluate("""
            ...
            var present = invoke(id, { value: 42 });
            var missing = invoke(id, null);
            (present === 42 && missing === undefined) ? 1 : 0;
            """);
    ...
    public async Task NamedMemberCallWithOptionalNamedPropertyReadArgument_UsesUnifiedBytecodeProductionFastPath()
    {
        ...
        var result = await engine.Evaluate("""
            ...
            var sink = {
                total: 0,
                add(v) {
                    this.total += (v === undefined ? 100 : v);
                    return this.total;
                }
            };

            var present = invoke(sink, { value: 5 });
            var missing = invoke(sink, null);
            (present === 5 && missing === 105) ? 1 : 0;
            """);
    ...
    public async Task IdentifierCallWithOptionalComputedThenNamedReadChainArgument_UsesUnifiedBytecodeProductionFastPath()
    {
        ...
        var result = await engine.Evaluate("""
            ...
            var present = invoke(id, { k: { value: 7 } }, "k");
            var missingHead = invoke(id, null, "k");
            (present === 7 && missingHead === undefined) ? 1 : 0;
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task AsyncFunctionDirectEvalDeclaration_UsesClassifiedDeclinedBodyRunner()
    {
        await using var engine = CreateEngine();
        var result = await engine.EvaluateAndAwait("""
            var asyncResult = undefined;
        ...
        Assert.Equal("fulfilled:undefined", result);
        var logs = CurrentLogger!.Collector.Snapshot();
        Assert.Contains(logs,
            static record => record.Message.Contains(
                "classified-async-function-declined-body-runner-residue",
    ...
    [Fact(Timeout = 5000)]
    public async Task ReturnImplicitArgumentsObject_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            returnArguments(41)[0];
            """);

        Assert.Equal(41d, result);
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=returnArguments argc=1",
                StringComparison.Ordinal));
    }

    [Fact(Timeout = 5000)]
    public async Task PassImplicitArgumentsObject_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            passArguments(function (args) { return args.length + ":" + args[1]; }, 41);
            """);

        Assert.Equal("2:41", result);
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=passArguments argc=2",
                StringComparison.Ordinal));
    }
    ...
    public async Task FinalRestArrowFunction_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var pick = (prefix, ...items) => items.length;
            pick(40, 1, 2);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task FinalRestArrowFunction_MissingRestArguments_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var count = (prefix, ...items) => items.length;
            count(42);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task SimpleLiteralDefaultParameter_WithImplicitArgumentsExplicitUndefined_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function inspect(value = 42) {
    ...
    [Fact(Timeout = 5000)]
    public async Task SimpleLiteralDefaultParameter_WithImplicitArgumentsProvidedValue_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function inspect(value = 42) {
    ...
    [Fact(Timeout = 5000)]
    public async Task SimpleLiteralDefaultParameter_WithImplicitArgumentsUpdate_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function inspect(value = 42) {
                arguments++;
                return value + ":" + typeof arguments + ":" + (arguments !== arguments);
            }

            inspect();
            """);

        Assert.Equal("42:number:true", result);
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=inspect argc=0",
                StringComparison.Ordinal));
    ...
    [Fact(Timeout = 5000)]
    public async Task SimpleLiteralDefaultParameter_WithImplicitArgumentsAssignment_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function inspect(value = 42) {
    ...
    [Fact(Timeout = 5000)]
    public async Task TryCatch_CatchOnlyFreeTypeOf_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var externalValue = 41;
        ...
        Assert.Equal("number:undefined", result?.ToString());
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=probe argc=0",
                StringComparison.Ordinal));
    ...
    [Fact(Timeout = 5000)]
    public async Task ArraySpreadWithDirectNamedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_ArraySpreadWithCapturedDirectNamedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeSpread(source, take) {
                return () => [...source.slice(0, take)];
            }

            var spread = makeSpread([40, 2, 99], 2);
            var result = spread();
            result[0] + result[1];
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task ObjectSpreadWithDirectNamedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
        ...
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=spreadObject",
                StringComparison.Ordinal));
    }

    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_ObjectSpreadWithCapturedDirectNamedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeSpread(source, take) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ArraySpreadWithReceiverOptionalNamedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function spreadNonSimple(source, take) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ArraySpreadWithReceiverOptionalComputedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            var result = spreadNonSimple([40, 2, 99], "slice", 2);
            result[0] + result[1];
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task ArraySpreadWithCalleeOptionalComputedCallSource_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function spreadNonSimple(source, method, take) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ObjectPropertyValueWithCalleeOptionalComputedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            var result = objectValue([40, 2, 99], "slice", 2);
            result.part[0] + result.part[1];
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task ObjectPropertyValueWithCalleeOptionalComputedCall_ShortCircuitsToUndefinedOnFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function objectValue(source, method) {
                return { part: source[method]?.(0, 1) };
            }

            objectValue({ }, "missing").part === undefined;
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task ObjectPropertyValueWithReceiverOptionalNamedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function objectValue(source, take) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ObjectPropertyValueWithReceiverOptionalNamedCall_ShortCircuitsToUndefinedOnFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function objectValue(source) {
                return { part: source?.slice(0, 1) };
            }

            objectValue(null).part === undefined;
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallArgumentObjectValueWithReceiverOptionalNamedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            callSink(function(value) {
                return value.part[0] + value.part[1];
            }, [40, 2, 99], 2);
            """);
        ...
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=callSink",
                StringComparison.Ordinal));
    }

    [Fact(Timeout = 5000)]
    public async Task CallArgumentObjectValueWithReceiverOptionalComputedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task CallArgumentObjectValueWithCalleeOptionalComputedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function callSink(sink, source, method, take) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ComputedObjectPropertyKeyWithCalleeOptionalComputedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task ComputedObjectPropertyKeyWithCalleeOptionalNamedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function objectKey(source) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ComputedObjectPropertyValueWithDirectNamedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            var result = objectValue([40, 2, 99], 2, "part");
            result.part[0] + result.part[1];
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task ComputedObjectPropertyKeyWithDirectNamedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_ObjectPropertyValueWithCapturedDirectNamedCall_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeObject(source, take) {
                return () => ({ part: source.slice(0, take) });
            }

            var build = makeObject([40, 2, 99], 2);
            var result = build();
            result.part[0] + result.part[1];
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task NonTerminalOptionalNamedPropertyDelete_UsesUnifiedBytecodeProductionFastPathAndNullIntermediateStillThrows()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var threw = false;
    ...
    [Fact(Timeout = 5000)]
    public async Task NonTerminalOptionalNamedPropertyDelete_NullIntermediateThrowRemainsCatchable()
    {
        await using var engine = CreateEngine();
        await Assert.ThrowsAnyAsync<Exception>(async () =>
            await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task NestedNamedReceiverComputedPropertyDelete_UsesUnifiedBytecodeProductionFastPathAndKeySemantics()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var hits = 0;
            ...
            var child = { value: 42 };
            var box = { child: child };
            remove(box, key) && hits === 1 && !Object.prototype.hasOwnProperty.call(child, "value");
            """);
    ...
    public async Task OptionalComputedPropertyDelete_UsesUnifiedBytecodeProductionFastPathAndSkipsNullishKey()
    {
        ...
        var result = await engine.Evaluate("""
            ...
            var child = { value: 42 };
            var removed = remove({ child: child }, key);
            var skipped = remove({ child: null }, key);
            removed + "|" +
                skipped + "|" +
                hits + "|" +
                Object.prototype.hasOwnProperty.call(child, "value");
    ...
    [Fact(Timeout = 5000)]
    public async Task OptionalComputedReadThenComputedPropertyDelete_UsesUnifiedBytecodeProductionFastPathAndSkipsNullishKeys()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var hits = "";
            ...
            var child = { value: 42 };
            var removed = remove({ child: child }, firstKey, secondKey);
            var skipped = remove(null, firstKey, secondKey);
            removed + "|" +
                skipped + "|" +
                hits + "|" +
                Object.prototype.hasOwnProperty.call(child, "value");
        ...
        Assert.Equal("true|true|first,second,|false", result);
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=remove argc=3",
                StringComparison.Ordinal));
    ...
    [Fact(Timeout = 5000)]
    public async Task NestedNamedReceiverComputedPropertyDelete_UsesUnifiedBytecodeProductionFastPathAndStrictSloppyFailureSemantics()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            var box = { child: {} };
            Object.defineProperty(box.child, "value", {
                value: 1,
                configurable: false
            });

            function sloppyRemove(box, key) {
                return delete box.child[key];
            }

            function strictRemove(box, key) {
                "use strict";
                return delete box.child[key];
            }

            var sloppyResult = sloppyRemove(box, "value");
            var strictThrew = false;
            try {
                strictRemove(box, "value");
            } catch (error) {
                strictThrew = error instanceof TypeError;
            }

            sloppyResult === false && strictThrew && Object.prototype.hasOwnProperty.call(box.child, "value");
            """);
        ...
        Assert.Contains(logRecords,
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=sloppyRemove argc=2",
                StringComparison.Ordinal));
        Assert.Contains(logRecords,
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=strictRemove argc=2",
                StringComparison.Ordinal));
    }
    ...
    [Fact(Timeout = 5000)]
    public async Task ParameterNamedArguments_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task LexicalArgumentsBinding_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function readShadow() {
        ...
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=readShadow argc=0",
                StringComparison.Ordinal));
    }
    ...
    [Fact(Timeout = 5000)]
    public async Task CallParameterNamedArguments_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            callShadow(function (value) { return value + 1; }, 41);
            """);

        Assert.Equal(42d, result);
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=callShadow argc=2",
                StringComparison.Ordinal));
    }
    ...
    [Fact(Timeout = 5000)]
    public async Task PrivateNamedPropertyReadAsBinaryOperand_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            class Holder {
    ...
    [Fact(Timeout = 5000)]
    public async Task PrivateReceiverPrefixNamedCall_PreservesReceiverAndUsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            class Holder {
                ...
                call(receiver, delta) {
                    return receiver.#child.value(delta);
                }
            }

            var holder = new Holder();
            holder.call(holder, 2);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task PrivateNamedPropertyUpdate_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            class Holder {
            ...
            var holder = new Holder();
            holder.update(holder, 0) + ":" + holder.read(holder);
            """);
    ...
    [Fact]
    public void SourceGate_AsyncFunctionDeclinedBodyRunner_IsClassifiedResidue()
    {
        var repositoryRoot = FindRepositoryRootForSourceGate();
        var sourcePath = Path.Combine(
            repositoryRoot.FullName,
    ...
    public async Task ArrowFunction_CapturedIdentifierCallExpression_WithCapturedArrayArgument_UsesUnifiedBytecodeProductionFastPath()
    {
        ...
        var result = await engine.Evaluate("""
            ...
            function probe(values) {
                "use strict";
                return this === undefined ? values[0] : 0;
            }
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_CapturedIdentifierCallExpression_WithCapturedObjectArgument_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeInvoker(fn, value) {
            ...
            function probe(box) {
                "use strict";
                return this === undefined ? box.item : 0;
            }
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_CapturedIdentifierCallExpression_WithCapturedTemplateArgument_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeInvoker(fn, value) {
    ...
    [Fact(Timeout = 5000)]
    public async Task FunctionExpression_CapturedOptionalIdentifierCallExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeInvoker(fn) {
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_CapturedOptionalIdentifierCallExpression_ReturnsUndefinedOnNullishFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeInvoker(fn) {
    ...
    [Fact(Timeout = 5000)]
    public async Task FunctionExpression_CapturedOptionalIdentifierCallExpression_ReturnsUndefinedOnNullishFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeInvoker(fn) {
    ...
    [Fact(Timeout = 5000)]
    public async Task FunctionExpression_CapturedIdentifierAssignmentExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeWriter() {
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_CapturedIdentifierCompoundAssignmentExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeWriter() {
    ...
    [Fact(Timeout = 5000)]
    public async Task FunctionExpression_CapturedIdentifierCompoundAssignmentExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeWriter() {
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_CapturedIdentifierUpdateExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task FunctionExpression_CapturedIdentifierUpdateExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeUpdater() {
    ...
    [Fact(Timeout = 5000)]
    public async Task ArrowFunction_CapturedIdentifierDeleteExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
    ...
    [Fact(Timeout = 5000)]
    public async Task FunctionExpression_CapturedIdentifierDeleteExpression_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function makeRemover() {
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedArrayObjectLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            sendNested(function(obj) { return obj.items[0] + obj.items[1]; }, 40, 2);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedObjectArrayLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            sendNested(function(items) { return items[0].value; }, 42);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedObjectObjectLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function sendNested(receiver, value) {
            ...
            sendNested(function(obj) { return obj.inner.value; }, 42);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedTemplateObjectLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            sendNested(function(obj) { return obj.label; }, "bytecode");
            """);

        Assert.Equal("hello bytecode", result?.ToString());
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=sendNested argc=2",
                StringComparison.Ordinal));
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedBinaryObjectLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function sendNested(receiver, a, b) {
            ...
            sendNested(function(obj) { return obj.total; }, 40, 2);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedBinaryArrayLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function sendNested(receiver, a, b) {
            ...
            sendNested(function(items) { return items[0]; }, 40, 2);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedComputedObjectBinaryValueArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            sendNested(function(obj) { return obj.answer; }, "answer", 40, 2);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedLogicalObjectLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            sendNested(function(obj) { return obj.value; }, 1, 42);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedLogicalArrayLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function sendNested(receiver, left, right) {
            ...
            sendNested(function(items) { return items[0]; }, 1, 42);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedConditionalObjectLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            sendNested(function(obj) { return obj.value; }, false, 10, 42);
            """);
    ...
    [Fact(Timeout = 5000)]
    public async Task CallWithNestedConditionalArrayLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            function sendNested(receiver, condition, consequent, alternate) {
            ...
            sendNested(function(items) { return items[0]; }, false, 10, 42);
            """);
    ...
    public async Task CallWithNestedBinaryTemplateArrayLiteralArg_UsesUnifiedBytecodeProductionFastPath()
    {
        ...
        Assert.Equal("total: 42", result?.ToString());
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=sendNested argc=3",
                StringComparison.Ordinal));
    ...
    [Fact(Timeout = 5000)]
    public async Task ObjectLiteralSpreadNamedPropertyRead_UsesUnifiedBytecodeProductionFastPath()
    {
        await using var engine = CreateEngine();
        var result = await engine.Evaluate("""
            ...
            readLiteral({ value: 42 });
            """);

        Assert.Equal(42d, result);
        Assert.Contains(CurrentLogger!.Collector.Snapshot(),
            static record => record.Message.Contains(
                "unified-bytecode-production-fast-path func=readLiteral argc=1",
                StringComparison.Ordinal));
    }
